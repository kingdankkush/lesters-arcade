...
| 🟡 PARTIAL | **Game SDK contract v1 + sandbox + HMH dogfoods it** | HMH runs as a sandboxed cabinet via the SDK; parent mediates all wallet ops; contract tests green | **SDK contract v1 DONE as a pure, tested module** (`apps/portal/src/arcade-sdk.mjs`, `tests/arcade-sdk.test.mjs`): `init(ctx)` lifecycle (`start/pause/resume/end/teardown/resize`), the full `arcade.*` event schema (§4.2) with per-event payload validators, `parseInboundMessage` security gate (source tag + SDK-major + gameId binding + schema), `createMessageRateLimiter` (§4.3), and `authorizeRankedSubmit` (free/ranked boundary + chain guard + mock-wallet exclusion). Also DONE: `game-manifest.mjs` schema + registry (§4.4) with a real `games/hard-money-heroes/game.manifest.json`. **Remaining:** wire HMH's in-process runtime to actually drive these lifecycle methods + emit the events, then move it into a sandboxed iframe cabinet (`allow-scripts`, no wallet/provider, CSP-pinned). The contract + validators are ready for that integration. |
