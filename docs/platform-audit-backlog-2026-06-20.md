...
|---|---|---|---|
| 🟡 PARTIAL | **Reconcile public-site canon** (iso roguelite + Crypto Wasteland) | No side-scroll language/art remains; copy matches current design | Player flow already says "Level 1: The Crypto Wasteland", isometric, auto-fire. Remaining drift is in: served news ticker (`PARALLAX SIDE SCROLLING`), hidden dev-backstage codex (`60fps Tactical Run-and-Gun`, `Jump / Double Jump`, combat sandbox), and `hmh-level-1-underchain-parallax` art caption. **This pass fixes the served public copy.** Legacy art JPGs still need an isometric Crypto Wasteland re-shoot (asset task). |
...
| ⬜ TODO | **Game SDK contract v1 + sandbox + HMH dogfoods it** | HMH runs as a sandboxed cabinet via the SDK; parent mediates all wallet ops; contract tests green | Not started. HMH is currently in-process, not an isolated sandboxed iframe cabinet. This is the keystone for third-party games. See `docs/LESTERS_ARCADE_PLATFORM_EXTENSIBILITY.md` + `docs/THIRD_PARTY_GAME_ONBOARDING.md` for prior thinking; needs the formal `init(ctx)` lifecycle + `arcade.*` postMessage event schema (§4.2) + manifest (§4.4). |

## P1 — high value, after P0

| Status | Task | Audit acceptance | Repo reality / notes |
|---|---|---|---|
| 🟡 PARTIAL | **Profile/Leaderboards hardening** | Name uniqueness/validation; avatar sanitize; cadence boards; practice/official separation; address normalization | Practice vs official separation ✅ (OPEN_QUESTIONS confirms boards stay separate; mock wallet can't submit official). Cadence boards configured (daily/weekly/monthly/yearly/all-time). **VERIFY/gaps:** display-name uniqueness + profanity + length validation; avatar sanitize (re-encode, strip metadata, type/size cap); wallet address case-normalization to avoid duplicate profiles. |
| ⬜ TODO | **Game registry + manifest + coming-soon cards** | Add-a-game = register a manifest; locked games render as roadmap | Coming-soon cabinets already render as cards/modals not dead links (OPEN_QUESTIONS confirmed). Formal `game.manifest.json` schema + registry not yet built. |
| 🟡 PARTIAL | **Perf pass** (lazy media, WebP/AVIF, code-split, SW caching) | Mobile load + Lighthouse targets met; game bundle loads only on cabinet open | Images already `loading="lazy" decoding="async"`; critical CSS inlined; stylesheets preloaded. **Gaps:** large generated JPGs not yet WebP/AVIF; splash video loads inline (not deferred); no service worker / `manifest.json` PWA caching yet; no per-route code-split of the game bundle. |
...
| Status | Task | Audit acceptance | Repo reality / notes |
|---|---|---|---|
| ⬜ TODO | **Third-party onboarding: public docs + reference adapter + test harness** | External dev can validate against a mock parent | Depends on SDK contract (P0). Draft thinking in `docs/THIRD_PARTY_GAME_ONBOARDING.md`. |
