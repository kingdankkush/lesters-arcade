...
- **HMH cabinet segment** (added 2026-09-25 by version-column, owner decision: no testnet season resets; every score shows its game version): the `lester-blaster` pattern also accepts an optional `:cabinet-\d+\.\d+\.\d+`, so it is `^site-\d+\.\d+\.\d+:game-\d+\.\d+\.\d+(?::cabinet-\d+\.\d+\.\d+)?$`. New HMH builds carry `HMH_CABINET_VERSION` (`apps/portal/src/hmh-cabinet-version.mjs`, first `0.5.0`; the HMH roadmap session owns bumps), and 1.8.x runs without the segment stay valid. The browser and the server share the one pattern in `RANKED_GAMES` (§7.1), which E3 (`bindRankedIdentity`) and E15 (`validateSeedBody`) apply.
  - **Version labels trust only shipped cabinets** (added 2026-09-25 by the version-column fixer, review finding): the client chooses the `buildHash`, and this format check cannot prove its cabinet, so the E5, E6 and E9 `versionLabel` (§4.3.5) shows an HMH or STACKED cabinet only inside the range the deploy has shipped (`SHIPPED_CABINETS` in `apps/portal/src/game-version-labels.mjs`: HMH `0.5` up to `HMH_CABINET_VERSION`, STACKED `0.2` up to `STACKED_CABINET_VERSION`, major.minor). Anything else, such as a future, oversized or never-shipped cabinet, reads `<Game> v?`. The server still accepts and stores such a build, which keeps this rule's promise never to reject a run for its build. E3 and E15 do not reject a cabinet newer than the server's either, so rolling back from a later cabinet bump to an earlier release that has this pattern still accepts the newer client's paid runs. *Corrected 2026-09-25 by the version-column fixer (round 2):* that holds only between releases that have the optional segment. A deployment without it (1.8.1 and earlier, or any release cut before version-column merged) rejects every HMH build that carries it with 400 `identity-buildhash-invalid`, which the client treats as a terminal rejection (`ranked-settlement.mjs`). The §13 rollback steps cover it. Chikun needs no range: its label comes from the `runtime_id` the server sets. The residual risk is in §12.
  - **Bumping the HMH cabinet** (rewritten 2026-09-25 by the version-column fixer, round 2): the HMH roadmap session bumps `HMH_CABINET_VERSION` alone when a balance change should show a new version. The labels read that constant and nothing else. `HMH_CABINET_VERSION` deliberately tracks the 0.5.x vertical-slice runtime, not the `1.0.0` in `games/hard-money-heroes/game.manifest.json`. Chikun and STACKED differ here: their cabinets equal their manifest versions. The HMH child's `RUNTIME_VERSION` (`apps/hmh-reboot/src/main.mjs`, `0.5.0` since the child shell, never bumped across balance cycles) is not tied to it. The earlier test that tied the two was dropped, because it would force every cabinet bump to edit the certified child runtime (AGENTS.md: a runtime edit is a new candidate). The bump has two visible effects. First, it is what makes `HMH v0.6` appear: until it lands, a run claiming cabinet `0.6.x` reads `HMH v?` (the shipped range above). Second, HMH `chain-index` rows, which have no build hash, change from `HMH v0.5` to `HMH v?` (§4.3.5).
...
- `versionLabel` (additive; added 2026-09-25 by version-column, owner decision: no testnet season resets, every score shows its game version) is `versionLabelFor(gameId, { buildHash: build_hash, runtimeId: runtime_id })` from `apps/portal/src/game-version-labels.mjs` for the ranked row: `HMH v0.5` (the cabinet major.minor of the HMH build hash; an HMH build without a cabinet segment is `HMH v0.5`), `Chikun v7` (from `chikun:canvas-runtime-v7`), `STACKED v0.2` (the cabinet major.minor), and `<Game> v?` when the version cannot be read. It is derived at read time from the migration-1 columns (nothing stored, no migration). The raw `build_hash` is never returned.
  - *Clarified 2026-09-25 by the version-column fixer (review findings; the `chain-index` rule revised in round 2 to match the relayed agreement that rows without a cabinet segment read `HMH v0.5`):* a `chain-index` row has no build hash. For HMH and STACKED it reads the game's only shipped cabinet while the game has shipped just one (`HMH v0.5`, `STACKED v0.2`: `SHIPPED_CABINETS` first equals current, and every ranked run so far was played on it). Once a second cabinet ships, such a row reads `<Game> v?`, because its version can no longer be told from the row. A later slice could date it by `confirmed_at` against the bump's release. A Chikun `chain-index` row reads `Chikun v7` because the indexer stores the reverse-mapped `runtime_id` (`server/indexer/index-chain.mjs`). A runtime the indexer cannot map is stored as its hex id and reads `Chikun v?`. `tests/version-column-api.test.mjs` runs the indexer and then reads E5, E6, E9 and E10 to prove all of this. An HMH or STACKED cabinet outside the deploy's shipped range (A11) also reads `<Game> v?`. A label is therefore always a version this deploy has shipped, or `v?`. The hosted board and profile mute `v?` with the tooltip "Game version unknown for this run", and the share page (E10) lists no version for it. A body without the field (cached before it existed) shows a muted dash on the board and no chip on the profile. The browser and the share page show a label only when it names the row's game (`versionLabelText(label, gameId)`, §7.10).
  - Not labelled: the `you` object above (the hosted "YOUR RANK" card, whose run is also the wallet's labelled row on the same board), the podium (whose runs are the labelled top three rows), and E6 `games[gameId].bestScore`. The version-column brief covers only board rows and profile runs, so these are left for a possible follow-up.
...
### 7.10 `apps/portal/src/game-version-labels.mjs` (version-column; browser and server)

Added 2026-09-25 by the version-column fixer (round 2). The module is pure, with no DOM, clock or environment. It imports only the one-line constants `hmh-cabinet-version.mjs` (`HMH_CABINET_VERSION`) and `stacked-cabinet.mjs` (`STACKED_CABINET_VERSION`).

```
export const GAME_VERSION_SHORT_NAMES = { 'lester-blaster':'HMH', chikun:'Chikun', stacked:'STACKED' };
export const HMH_PRE_CABINET_VERSION = '0.5';  export const UNKNOWN_GAME_VERSION = 'v?';
export const SHIPPED_CABINETS = { 'lester-blaster': { first:'0.5', current:<HMH_CABINET_VERSION major.minor> },
                                  stacked: { first:'0.2', current:<STACKED_CABINET_VERSION major.minor> } };   // A11
export function versionLabelFor(gameId, { buildHash, runtimeId } | null, { cabinets } = SHIPPED_CABINETS) → '<Game> v<version>' | '<Game> v?' | 'v?'
    // server (server/neon/rows.mjs rowVersionLabel → E5, E6, E9); never throws; rules in §4.3.5 and A11
export function versionLabelText(value, gameId = null) → value | null
    // browser and share page: accepts only the shapes versionLabelFor writes, with one of the three names;
    // given gameId, only that game's label (a known game never shows a bare 'v?')
export function versionLabelTitle(label | null) → 'Played on <label>' | 'Game version unknown for this run' | 'Game version unavailable'
```

The browser calls only `versionLabelText` and `versionLabelTitle`, and only from lazy chunks. The `/* @__PURE__ */` marks on `SHIPPED_CABINETS` let esbuild drop it from the browser bundle. esbuild splits chunks by file, though. Both the lazy label chunk and the first-load `arcade-core` import the two constants, so they sit in a 51 B chunk of their own that the portal loads first. That is one extra request, and no budget of §11 rule 5 changes. To remove it, the module would have to be split, or every server caller would have to pass the ranges. The brief asks for one shared module, and a forgotten range argument would silently show `v?`, so neither was done.
...
4. **Site rollback:** Vercel **Instant Rollback** to `dpl_2Q1MYFG9YQWypPjs84VLKdTkwdsu` (1.7.0). Never "Redeploy" or rebuild a pre-1.8.0 commit with production env. Even then, the renamed secrets mean old code finds no keys and fails closed.
   - **Rolling back past version-column** (added 2026-09-25 by the version-column fixer, round 2): a deployment without the HMH cabinet segment (1.8.1 and earlier, or any release cut before version-column merged) rejects the HMH build hash that release's clients send (`…:cabinet-0.5.0`, A11) at E15 and E3 with 400 `identity-buildhash-invalid`. `ranked-settlement.mjs` treats that as a terminal `rejected`. So an HMH Ranked run that was paid on the newer client, and that reaches E3 while the old deployment serves (in flight, or "saved on this device" and retried), loses its entry. The seed ticket comes before payment, so new entries are refused before anyone pays. Steps:
     - Prefer a rollback target from version-column on, which has the segment.
     - If the target predates it, pause Ranked first (step 1). New entries then stop before payment. Paid runs are saved on the device, because E3 answers 503, instead of being rejected.
     - Re-promote a release with the segment before Ranked resumes, so saved HMH runs retry against it.

     Chikun and STACKED are not affected.
