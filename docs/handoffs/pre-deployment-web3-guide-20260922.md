...
- `contracts/deploy-config.testnet.json` still lists the **old June deployer** `0x24501ad9…20d4` for every role. The edit to the addresses above was blocked by the classifier and reverted. Target values:
  ...
  > **Amended 2026-09-26 (owner decision):** the Ranked entry fee is now **0.01 zkLTC** (from 0.1 zkLTC: the LiteForge faucet gives 0.05 zkLTC per request, and 0.1 zkLTC was about $7.31 at that day's Litecoin price). The reserve stays 0.002 zkLTC, so a run totals **0.012 zkLTC**: `"entryFeeWei": "10000000000000000"` for every game in `contracts/deploy-config.testnet.json`, client `RANKED_ENTRY_FEE_WEI = '10000000000000000'` in `apps/portal/src/arcade-core.mjs` (the single source every displayed fee string derives from; `tests/ranked-fee-source-of-truth.test.mjs`), and the settle floor `DEFAULT_MIN_PAID_WEI = '12000000000000000'` in `server/config.mjs` (contract A27). **Release order:** the server with the lower floor ships **first**, because runs paid at the old 0.102 zkLTC still clear a 0.012 floor; only once it is live does the pre-deploy session send the three `GameRegistry.setEntryFee(gameId32, 10000000000000000)` transactions from the operator wallet `0x6Ac08Bed727A6951D755F0674f096E6A8AC06bfF` (`node scripts/operator-actions.mjs entry-fee 10000000000000000` plans them and refuses a fee whose total is under this checkout's floor; `--broadcast --confirm SET_ENTRY_FEE_4441` with the operator key sends them). The reverse order would refuse every new 0.012 run at settle (`402 entry-underpaid`). Until the transactions land, the modal's Entry row (the client constant, 0.01) and its quoted reserve and total (the chain, 0.002 and 0.102) disagree for that window; the ranked-onboarding slice renders the whole breakdown from the live quote. **Jackpot:** pots are funded only by $CHIKUN deposits (`minFundWei` 100 tCHIKUN) and fees never flow into them; the coupling is eligibility, and the deployed testnet epoch carries `minPaidWei` 0.1 zkLTC (J17), so no 0.012 run qualifies there. The jackpot is on hold until mainnet (owner decision 2026-09-26): no action now, and the mainnet instance must be deployed with `minPaidWei` equal to the fee in force then (or its rules rescheduled on a Monday boundary). **Relayer:** healthy; each run brings the 0.002 reserve, and a settle costs about 0.0001 zkLTC at 0.22 gwei (about 0.0007 at the 1.45 gwei peak). The 85/15 split becomes 0.0085 zkLTC developer / 0.0015 zkLTC treasury per run. Every 0.1 / 0.102 figure below is superseded; the hand-over is `docs/handoffs/ranked-fee-20260926.md`.
...
- The modal shows: entry 0.01 zkLTC, settlement reserve 0.002 zkLTC, total 0.012 zkLTC (amended 2026-09-23 and 2026-09-26; the reserve and total follow the live `quoteEntry`), and where the money goes (85% developer / 15% arcade). There is **one** wallet confirmation.
