...
## 3.4 Second pass (2026-09-12): five gameplay and feedback slices

Built as a multi-agent package: eight read-only investigations mapped the exact code paths, a ranking pass wrote bounded scopes, and each slice went through an implementer, three adversarial reviewers (determinism, correctness, scope), a fix round and a release-gate verifier in its own worktree before being merged here. Every slice carries node tests; the merged tree passes the unchanged release gate. Initial child JS after the package: 1,041,489 B of 1,048,576 B (6.9 KB headroom); each slice's cost is recorded in the release receipt.

| Slice | Roadmap rows | What the player now sees | Boundary |
| --- | --- | --- | --- |
| Reload presentation (`reload-presentation.mjs`, HUD card) | §10 reload presentation, §3 combat feedback | On the aim and hurt poses the weapon layer dips and tilts over the first quarter of a reload and snaps level on the completion tick; shotgun, machine gun, rail and launcher props dip the same way. The cockpit card gains an amber progress bar under the pips, an ammo-low tint on the last quarter of a clip, a red reserve when the pocket cannot fill another clip, a one-frame ring pulse and a six-tick muzzle glow on completion. | Projection only. The proposed dry-fire click was removed after review: the shipped input model has no manual trigger, so the click could only fire on phantom edges. |
| Enemy hit feedback (`enemy-hit-feedback.mjs`) | §8 readable reactions, §10 armor-break feedback | A landed hit flashes the body pale red for two ticks, knocks it against the blow and eases back, squashes it in proportion to damage and knockback resistance, and sprays grey or blue shards from armoured, shielded or support-pulsed bodies. A hit that lands during a tell or strike is now visible. | Projection only; the enemy soak hash is unchanged. Tell, strike and recovery ticks for every archetype are now measured from the real step function and pinned in `tests/hmh-reboot-enemy-tell-timing.test.mjs`; release telemetry publishes the last measured tell-to-strike. |
| Simulated hazards (`world-hazards.mjs`) | §12 hazards, §8 "hazardous role changes terrain and warns", §13 machinery | The ravine rockfall warns for 1.1 s (collapsing ring, swelling slab shadows, a caption when close) and lands 24 damage every 5 s; the liquidation grid blinks faster and pulses 14 damage every 4 s; the hashwood spore bed slows hero and enemies to 55%; the mining conveyor pushes hero and enemies 150 units/s along its axis. Dashing dodges the hits; evidence tours are exempt. | Simulation, deterministic by construction (pure functions of the integer tick and frozen anchors, no RNG). Hazards damage the boss but stop at 1 HP so every boss defeat still flows through the player-weapon authority path. Environmental kills award no XP or attribution. Explosive zones and destructibles were deliberately deferred to the next session. |
| Run recap (`hmh-run-recap.mjs`, run-summary schema 6) | §15 "post-run explanation of build, milestones and cause of defeat", §17 profiles | The game-over panel shows the cause of defeat (killer archetype, boss attack, hazard or own grenade with damage and clock), the build (weapons with kill counts, augments with ranks), milestones (level-ups, machinery operated, secrets found, boss engaged) and the run row (seed, max combo, level, survival). The Bosses metric now reads the real count. Phones stack the rows inside the scrollable panel. | Schema 6 is additive; v1 to v5 records still validate and render. The child accumulator records the first killing hit and milestones; the bridge envelope stays far below its cap. |
| Power-up timers (HUD chips) | §14 duration, stacking and expiry | Each active chip carries its effect's colour and accent, drains continuously across its 600-tick window, pulses in the last two seconds (the aura breathes in step), pops once on a refresh, and an expiry cue plays when a buff ends. The briefing states that a repeat pickup restarts the timer. | Projection only; an unchanged frame performs zero DOM writes; reduce-motion and reduce-flash paths covered. |

Stale evidence scripts corrected along the way, each proven pre-existing on the base commit: the boss-presentation smoke's audio assertion (`hmh-boss-phase.wav`), the combat smoke's level-up pause during the roster wait, the collectible smoke's XP and enemy-count expectations under the shipped opening balance (the pickup's own award is now published as `collectibleLastXp`), and the hero-selector smoke's rest-frame index (the select screen has shown the portrait strip since `3e45dfa4`). That selector smoke then stops at its mobile height guard (1,866 px against an 1,800 px limit), identically on the untouched base with the same corrected script, so the phone select page is recorded as a pre-existing layout item for the owner's mobile pass rather than a regression here. The combat smoke's later steps still drive the retired manual weapon keys and remain red; the desktop portal end-to-end `pause-resume` and `mid-run-restart` flows and five mobile-profile flows fail identically on the base (the mobile pause-menu music player intercepts the menu button) and are recorded, not fixed, here.
