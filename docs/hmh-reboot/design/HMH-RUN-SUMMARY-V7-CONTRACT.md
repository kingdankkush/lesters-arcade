...
- **1.8.4 exception (16.5).** Four consistency rejects are added to the v6 path for fabricated inputs only: `grenade-kills-above-weapon-kills`, `pickups-above-capacity`, `district-path-invalid` and `districts-before-travel-time`. With those four flags removed, the 288-case corpus still hashes to the `60ea173a` digest.
...
4. **The 1.8.4 pickup and district rules for v7.** The v7 path has the loose `collectibles-above-capacity` (21 placements × re-arms, over all effects) and only the soft `node-in-unvisited-district`. Pin the v7 child's placement table (per effect, with re-arm ticks) and its district strips and entries into `sdk/hmh-run-contract-v7.mjs`, and mirror `pickups-above-capacity`, `district-path-invalid` and `districts-before-travel-time` (16.5). Otherwise a schema-7 run gets back the gaps the v6 path closed in 1.8.4.
