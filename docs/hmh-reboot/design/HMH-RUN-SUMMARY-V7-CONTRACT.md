...
- **1.8.4 exception (16.5, 16.6).** Twelve consistency rejects are added to the v6 path for fabricated inputs only: `grenade-kills-above-weapon-kills`, `pickups-above-capacity`, `district-path-invalid` and `districts-before-travel-time` (16.5), then `activity-without-time`, `equipped-ticks-above-run`, `weapon-without-source`, `grenade-kills-above-contacts`, `grenade-detonations-above-launches`, `grenades-thrown-above-supply`, `damage-dealt-mismatch` and `combo-above-kills` (16.6). With those twelve flags removed, the 288-case corpus still hashes to the `60ea173a` digest. Round 3 adds four melee-trail rejects (16.7), and the digest holds with all sixteen removed.
...
6. **The bridge and the portal** (`sdk/hmh-bridge-protocol.mjs`, `apps/portal/src/hmh-run-history.mjs`, `apps/portal/src/hmh-reboot-bridge.mjs`): they validate run summaries with the base schema module, which accepts schema 1-6 only (16.1). The child branch switches them to `sdk/hmh-run-summary-schema-v7.mjs` when the child starts emitting schema 7, and funds the bytes (about 5.8 KB minified at 16.1; 8,029 bytes after 16.4) from its own HMH initial-JS budget. Only 1,815 bytes of headroom were left at `da3c0756` (16.1), so the move needed about 6.2 KB (8,029 − 1,815) of HMH initial-JS cuts. On the integration head `b7c8307a` (1.8.4's HMH perf steps 1–2 on the release line) the headroom with this branch is 897 bytes, so the move now needs about 7.1 KB (8,029 − 897) of HMH initial-JS cuts first, or the v7 module loaded lazily, off the initial path.
...
### 15.1 Opening the schema-7 gate (item 1): preconditions

Item 1 must not land until all of these hold. Each can land earlier; the ones marked **landed** are in this branch.

1. **Only in the deploy that serves the 1.9.0 child.** The build hash is client-chosen and only format-checked (D11), so while any deploy accepts schema 7, a fabricated run can name `game-1.9.0` and take the v7 rules. Open `server/verify/hmh.mjs` to schema 7 only in the same deploy that serves the first schema-7 child. If there is ever a window where that cannot hold (schema 7 accepted while a 1.8.x child is served, or a schema-7 child shipped below 1.9.0), first add a server check that the build hash's game version is no higher than the deployed `GAME_VERSION` (`apps/portal/src/version-tracking.mjs`), in both `validateSeedBody` (`server/settle/seed.mjs`) and `validateRankedIdentity` (`apps/portal/src/ranked-identity.mjs`).
2. **Fixtures and an honest corpus from the real v7 accumulator.** Rebuild both v7 fixtures from the child's v7 accumulator (today they are the 1.8.1 accumulator's summary extended to schema 7, 16.3), and add a multi-seed honest-run corpus for the v7 path, like `tests/fixtures/ranked/hmh-honest-corpus.mjs` for v6 (16.5), that must come back not rejected.
3. **Grenade kills, landed.** `grenade-kills-above-weapon-kills` rejects on the v7 path as on v6: `grenades.kills` above the kills of `satoshi-frag` plus `launcher-rig`. A schema-level equality can come with the child's v7 accumulator.
4. **The 1.8.4 consistency rules for v7.** The v7 path has the loose `collectibles-above-capacity` (21 placements × re-arms, over all effects) and only the soft `node-in-unvisited-district`. Pin the v7 child's placement table (per effect, with re-arm ticks and unlocks) and its district strips and entries into `sdk/hmh-run-contract-v7.mjs`. Then mirror `pickups-above-capacity` with the unlock gates of 16.6, `district-path-invalid` and `districts-before-travel-time` (16.5), and the eight second-round rejects of 16.6 (ticks, weapon sources, grenade trail, damage, combo) against the v7 child's loadout, grenade supply and combo. Otherwise a schema-7 run gets back the gaps the v6 path closed in 1.8.4.
5. **The remaining review tests, landed.** S14 and S15 per kind at their boundary; the S10 reserved rows with `applied = 1, offered = 0`; `boss-before-ready` at `firstInitiatedTick = 0`; one case per soft-flag sub-condition; each half of S8 on its own, with the milestone-only variant of the A2-1 K1 payload; the v7 ceilings, kill capacity and cache room pinned as literals; a hashed 172-case v7 mutation corpus.
6. **The bridge and portal bytes** (item 6): about 7.1 KB of HMH initial-JS cuts on `b7c8307a` (6.2 KB at `da3c0756`), or a lazy load.
...
- **Cost.** The factoring adds 48 bytes to the HMH initial JS (1,046,713 → 1,046,761; 1,815 bytes of headroom left); the child entry is unchanged (360,738 of 480,000). The V7 catalogues reach the portal's `main.js` only, through `achievements/stats.mjs` (+3,030 bytes to `main.js`, measured by the integration review; the first estimate here was about 2.2 KB), against the 2,710,000-byte main-bundle budget of `scripts/hmh-load-speed-report.mjs`; `main.js` measured 1,176,793 bytes at `60ea173a`), outside the HMH budget. On the integration head `b7c8307a` the build measures HMH initial JS + shared at 1,047,679 bytes with this branch against 1,047,594 without it (+48 from the base schema module, +37 in the child entry from the `evidenceSafe` change of 16.6), so 897 bytes of headroom.
...
### 16.4 Red-team round (three attacks against `ff265ab8`, which the rebase onto `da3c0756` made `3003a565` and the rebase onto `b7c8307a` made `4f5023f4`)
...
| A0-2, A1-1 (medium): schema 7 accepted under any build hash, so a 1.8.x-labelled run could carry v7 rules | **Fixed.** `build-predates-schema-7` (D11). The reverse direction (reject schema 6 at 1.9.0 or later) is **not adopted**: the build hash is the portal's, and a newer portal can host a 1.8.x child cached by the service worker, which owner override 2 keeps valid. **This labels honest runs only:** the build hash is client-chosen and only format-checked (`server/settle/seed.mjs:47`, `validateRankedIdentity`), so a fabricated run can name `game-1.9.0`. The gate stays safe only under the 15.1 precondition 1 | `v7: a schema-7 summary from a build older than the first v7 child rejects` |
...
| A0-6 (low, inherited from v6): `collectibles[*].collected` unbounded (a million Railgun cores funding level 1,000; 250 Berserk Candles) | **Fixed for v7** (`collectibles-above-capacity`, 5.1). **Fixed for v6 in 1.8.4** (`pickups-above-capacity`, 16.5), per effect from the 1.8.x placements | `v7: more collectible pickups than the placements can give reject` |
...
- A schema-6 summary can carry any build hash (D11), and a schema-7 summary can name any game version the format admits (15.1, precondition 1).

### 16.5 The 1.8.4 v6 consistency rules (a live 1.8.2 issue)

**The issue.** Three summary fields reach the achievement stats directly (`statsFromHmhRunSummary`: `grenadeKills` from `grenades.kills`, `powerUpsCollected` from `collectibles`, `districtsVisited` from `exploration.visitedDistrictMask`) and no v6 ceiling bounded them. From `hmh-valid` (36 kills), setting only `grenades.kills = 250`, bonus-life `collected = 250` and `visitedDistrictMask = 63` verified `ok` with no flags in 1.8.2, and that one paid run earned grenade-demolitionist, powerup-hoarder, grenade-century, slums-clear and foundry-clear. Because schema 6 is accepted under any build hash, the same path would also have sidestepped the v7 rules.

**The fix** (`server/verify/hmh-plausibility.mjs`, `HMH_V6_CONSISTENCY_RULES`): four rejects on the v6 path, answered as `422 implausible-run` before any stats exist. The 1.8.x child (`apps/hmh-reboot/src/**`, `sdk/hmh-run-summary.mjs`) is identical from 1.8.0 to 1.8.3, and every literal is pinned against it by a parity test.

| Reject | Formula (T = the run's ticks) | Why every honest run passes | Margin |
|---|---|---|---|
| `grenade-kills-above-weapon-kills` | `grenades.kills > byWeapon[satoshi-frag] + byWeapon[launcher-rig]` | `recordRunKill` adds a grenade kill exactly when the killing weapon is one of those two, and nothing else writes the count, so they are equal | None needed: only the excess rejects |
| `pickups-above-capacity` | for some effect e, `collected_e > Σ over e's placements of (1 + ⌊T / rearm⌋)` (a one-shot placement counts 1). Value and limit: the offending effects' pickups and capacity | A pickup is only a `collectible:collected` event of `stepCollectibles`, which gives a placement once, or again only after its re-arm ticks. The 21 placements: 10 authored points of interest (6 re-armed every 10,800 ticks), 3 seeded one-shot weapon events, 8 objective rewards (3 re-armed at 7,200, 1 at 10,800) | Every placement counts as available and unlocked from tick 0 (a real run starts at tick 1, and events and objective rewards unlock later) with no travel between |
| `district-path-invalid` | the visited mask is not 0 and not one contiguous run of strips containing the entry strip of `selectLevelEntry(seed)`. Value: the mask; limit: the entry's bit | The six strips tile the world along x; every entry is at least 250 units inside its strip; one tick moves the player far less than a strip is wide. So the first recorded tick is in the entry strip and every later strip is entered from a neighbour | None needed: a structural property |
| `districts-before-travel-time` | `T × 48 + 480 < travel`, where travel is the shortest walk from the entry to both ends of the visited strips (`L + R + min(L, R)`). Value: T; limit: the fewest ticks that cover it | 48 px is twice a dash tick (192 px over 8 ticks = 24 px), and a running tick moves under 20 px even at the maximum movement ranks (× 1.68), time dilation (× 1.2) and the steepest downhill (× 1.04) with the conveyor (150 px/s) and a fresh maximum recoil or knockback impulse (74 px/s, 8.9 px in all) every tick | 2× the fastest tick, plus 480 px once. The legal minimum for all six districts is 168 to 226 ticks by entry; the fastest honest full-map pilot takes 3,890 |

All six districts in a 3-minute run from the relay stays legal: it is honestly reachable (9,200 units at 240 units/s take about 38 s), so no rule short of a replay can refuse it.

**The honest corpus.** `tests/fixtures/ranked/hmh-honest-corpus.mjs` plays 75 schema-6 runs (five pilots × three seeds per level entry, 900 to 110,000 ticks, 3.6 million in all) through the real 1.8.x accumulator and run-progression, and the child's own level entry, district strips, movement and dash (open ground, always downhill), collectible placements and `stepCollectibles`, machinery sites, and the director's insertion schedule and archetype choice; combat is scheduled within the child's grenade supply. The pilots aim at the rules: a sprinter dashes between the map's ends with every movement rank offered, a hoarder chases every available placement and site, a grenadier kills with grenades whenever it holds one, a wanderer roams and fights the Liquidator, a camper stays at its entry. `tests/server-verify-hmh-honest-corpus.test.mjs` pins the digest and requires every run to be schema-valid, never rejected and free of consistency flags, with each rule's neighbourhood reached: grenade kills equal the grenade weapons' kills in every run (117 at most); the fullest run collects 89.7% of its pickup capacity and 80 effect rows sit exactly at their capacity; every entry has a full-map run; the pilots' longest tick (a dash tick, 24 px) is half the budget. The ceilings come from the child's rules, never from the corpus.

**Other tests.** Parity pins for every literal; each rule at its boundary; the fastest possible collector through the real `stepCollectibles` meets an exact per-placement count that never passes the capacity; every mask from every entry; the 1.8.2 payload end to end through `verifyRankedRun` (422). The fixture builder walks districts from the seed's entry strip, so rehearsal and settle runs at any seed stay contiguous; the committed fixtures rebuild byte for byte.

**The frozen v6 corpus.** With the four consistency flags removed, the 288 results still hash to the `60ea173a` digest. With them, seven results gain a flag and two change verdict: tripling every cache pickup in the realistic and level-90 runs claims three Hash Rail Cores (two one-shot placements) and rejects `pickups-above-capacity`. The other five were already rejected and add `districts-before-travel-time`. The new digest is `V6_CORPUS_DIGEST_1_8_4`.

**Residuals.**
- Grenade kills can still reach the run's kill total, by crediting every kill to a grenade weapon. The 1.8.x summary has no per-throw record, and a per-blast target cap is not a 1.8.x rule.
- Districts in a long run are free within the contiguity rule, as above.

**Superseded in part by 16.6.** The second round keeps these four rejects and tightens `pickups-above-capacity`: a placement now counts only from its first tick (see 16.6), not from tick 0 and unlocked. The first residual above narrows: a grenade kill now needs a consistent grenade trail.

### 16.6 Second red-team round (two attacks against `a9663a09`, which the rebase onto `b7c8307a` made `7668da79`)

Every reproduced hole has a failing test built from its payload, now passing. "R0-3" is attack 0 (false negatives in the 1.8.4 rules), finding 3; "R1" is attack 1 (the new rules against real 1.8.x runs, and `evidenceSafe`). The attacks ran every payload through the real `verifyRankedRun` and `deriveEarnedAchievements`.

| Finding | Disposition | Where |
|---|---|---|
| R0-1 (high): a zero-tick run verified with every placement once, two districts, 20,000 damage and three weapons equipped, and earned seven achievements (P2); with `maxCombo` 30 it earned the combo ones too (P2b); the twelfth such run earned powerup-hoarder (P3) | **Fixed.** `activity-without-time`, `equipped-ticks-above-run`, `combo-above-kills`, and `pickups-above-capacity` counts nothing before tick 1 | `red team: zero-tick runs record nothing (P2, P2b, P3)`; `the round-2 red-team payloads are a 422 before any achievement stats` |
| R0-2 (high): `pickups-above-capacity` counted placements still locked: the Liquidator vault with no boss kill, site rewards with no site operated, seeded events before their minimum tick (P7, 18 pickups in 10 s); powerup-hoarder in one 60-minute run (P3b) | **Fixed.** A site reward counts from its site's operated tick, the vault only with a Liquidator kill (from the boss band), a seeded event from its minimum tick. Residual below: a site milestone is itself a claim | `red team: pickups from locked placements reject (P7, P3b)`; `pickups above what the unlocked placements can give reject, per effect and at each boundary` |
| R0-3 (high): grenade kills credited to a grenade weapon with no pickup, launch, throw, detonation or contact: grenade-demolitionist in one 6.5-minute run (P1'), hard-fork-hero in 50 s (P11) | **Fixed as far as the summary allows.** `weapon-without-source`, `grenade-kills-above-contacts`, `grenade-detonations-above-launches`, `grenades-thrown-above-supply`. A forger must now fake a consistent trail. A 1.8.x blast has no target cap, so that stays open (a residual, pinned by a test) | `red team: grenade kills need a grenade trail (P1', P11)`; `residual: a consistent fabricated grenade trail still verifies` |
| R0-4 (medium): weapon achievements for weapons never picked up (P8), or that can never be active (P8b) | **Fixed.** `weapon-without-source` | `red team: weapons never picked up, one damage field and a combo above the kills reject (P8, P8b, P9, P10)` |
| R0-5 (medium): damage-chain from `totals.damageDealt` alone (P9) | **Fixed.** `damage-dealt-mismatch`. Residual: damage has no time ceiling, so a consistent `weapons[].damage` trail still reaches 20,000. A sound ceiling would be loose anyway: a nuke hits every enemy on the field for 999 | same |
| R0-6 (medium): flags give achievements no protection: max-combo-30 from 5 kills, flagged only (P10) | **Fixed for the payload.** `combo-above-kills` rejects what was only the 1.8.1 flag. Withholding achievements from every flagged run is **not adopted**, because near-ceiling flags fire on honest skilled runs. Owner decision | same |
| R0-7 (low): the district rule is loose at its edges: two districts at T = 0 from three entries (P4); foundry-clear from the yard at T = 82 (P4b) | **T = 0 fixed** (`activity-without-time`). The per-tick budget is **kept**: 48 px a tick and 480 px once is 16.5's deliberate margin of twice the fastest tick. The cooldown model would move the fabricated foundry-clear minimum from 82 to about 211 ticks, 2 s of play against the honest full map's 3,890 ticks, and would rest on a hand bound with no margin | `visited districts farther from the entry than the run's ticks can cover reject, at the boundary` |
| R0 informational: a Liquidator kill at exactly tick 72,000 (P5); 3 kills at T = 1, flagged (P6); survival achievements by waiting | **Recorded, not changed.** v6 has no minimum Liquidator fight (v7 has the 180 ticks of 7.2); the wall-clock bound (A26) makes P5 take 20 real minutes; P6 earns first-blood only. Waiting is what the survival achievements count | – |
| R1-1 (reproduced; a regression of 16.5): `district-path-invalid` rejected the evidence-safe terminal-pilot Ranked run for 33 of 40 seeds. `scripts/ranked-live-browser-e2e.mjs` loads that run (`?evidenceSafe=1&terminalPilot=1`) to certify HMH settlement, and under `evidenceSafe` the child spawned at the Frontier Relay for every seed | **Fixed in the child, not the verifier.** A Ranked session now spawns at `selectLevelEntry(seed)` whatever the URL says (R1-2), so the e2e's run is an ordinary two-tick Ranked run at the seed's entry and verifies at every entry. No default-spawn exception was added: the only runs it would admit are R1-2's invulnerable ones | `the evidence-safe terminal-pilot Ranked run (the Ranked e2e) verifies at every level entry` |
| R1-2 (from the code; live since 1.8.0): `?evidenceSafe=1` on the portal made a Ranked hero invulnerable to everything but the Liquidator (the host forwards it in any mode), and nothing in the summary marks it | **Fixed in the child.** `evidenceGameplayEnabled = evidenceSafeEnabled && payload.mode !== 'ranked'`, set in `initializeSession`, drives the evidence spawn and the invulnerability. Free evidence runs and every evidence smoke keep both (standalone sessions are Free). The terminal pilot still ends an evidence run on tick 2 in any mode. The host is unchanged. Residual: a 1.8.x child cached by the service worker behaves as before until it updates | `evidenceSafe changes neither the spawn nor the hero's vulnerability in a Ranked session` |
| R1 negative results (grenade equality, the pickup bound, the district rules, the fixture plans at 23 seeds, child code identity from 1.8.0 to 1.8.3) | Consistent with this round: the 75-run honest corpus and the fixture plans from every entry meet all twelve rejects | `tests/server-verify-hmh-honest-corpus.test.mjs`; `the v6 fixture plans meet every consistency rule from every level entry` |

**The second-round rules** (`HMH_V6_CONSISTENCY_RULES`; T is the run's ticks). Each is an exact invariant of the 1.8.x child, pinned by `the second-round consistency literals and the accumulator facts they rest on equal the 1.8.x child`. Only the excess rejects, so none needs a margin beyond the one stated.

| Reject | Formula | Why every honest run passes |
|---|---|---|
| `activity-without-time` | T = 0 and (`visitedDistrictMask` ≠ 0 or `damageDealt` > 0). Value: how many of the two | The simulation's first step is tick 1. `recordRunTick` sets the district bit and `recordRunDamage` runs inside a step |
| `equipped-ticks-above-run` | Σ `weapons[].equippedTicks` > T | `recordRunTick` adds one to the active weapon's row per recorded tick, and recorded ticks rise strictly from tick 1 to at most the end tick |
| `weapon-without-source` | any weapon row that is: equipped though not in `WEAPON_ORDER`; a cache weapon (all of `WEAPON_ORDER` but the Coin Blaster) equipped or credited a kill with 0 pickups; picked up more often than its cache effect was collected, or picked up with no cache at all; Satoshi Frag with kills and 0 thrown; Nuke Liquidation with kills and no `nuke-liquidation` collected. Value: the offending rows | The loadout is `WEAPON_ORDER`, and only the Coin Blaster is owned at the start. `grantWeaponPickup` is the one grant. It is reached only from a weapon-cache pickup, which records the collection too, or from the evidence-only `weaponPilot`, which the portal never forwards. A grenade weapon's hit comes only from a blast, and a Nuke's only from the nuke event |
| `grenade-kills-above-contacts` | `grenades.kills` > `grenades.contacts` | A grenade weapon's kill comes only from a blast hit, and each non-player blast hit is one contact |
| `grenade-detonations-above-launches` | `grenades.detonated` > `grenades.thrown` + `weapons[launcher-rig].triggers` | A grenade exists only through `throwGrenade`. A hand throw records `thrown` when it spawns, and each launcher shot records a trigger, spawned or not |
| `grenades-thrown-above-supply` | `grenades.thrown` > 3 + min(3, `cold-storage` selected) + `nuke-liquidation` collected | Three charges at the start, one per Extra Grenade rank, one per recharge. Only the Nuke and the Scrypt Cache recharge, and both are the `nuke-liquidation` effect. The charge cap is ignored (margin) |
| `damage-dealt-mismatch` | `totals.damageDealt` ≠ Σ `weapons[].damage` | `recordRunDamage` adds the same whole amount to both (combat-events rounds it) |
| `combo-above-kills` | `totals.maxCombo` > `kills.total` | The combo rises by exactly one per recorded kill: `awardComboXp` follows each `recordRunKill`. The 1.8.1 flag `combo-exceeds-kills` stays beside it |

**`pickups-above-capacity`, second round.** A placement `[rearm, unlock]` gives nothing before its first tick f = max(1, unlock), then 1 + ⌊(T − f) / rearm⌋ (1 for a one-shot). The unlock is:
- 0 for an authored point of interest;
- the event's minimum for a seeded event (3,600, 7,200 or 10,800; the seeded tick is never earlier);
- for a site reward, the site's `milestones.sites[].tick` if it operated (main.mjs unlocks the reward in the step that records the milestone), else locked;
- for the vault, 72,000 if the run claims a Liquidator kill (the kill unlocks it), else locked.

The fastest collector through the real `stepCollectibles` meets this count exactly wherever an effect has no seeded event.

**The child change** (`apps/hmh-reboot/src/main.mjs`, 1.8.4) is three lines. The evidence spawn is kept in `evidencePlayerSpawn`, and `evidenceGameplayEnabled` replaces `evidenceSafeEnabled` for the session's spawn and the hero's invulnerability. The accumulator, the summary and every value a rule reads are unchanged, so the v6 rules keep holding for the 1.8.0–1.8.3 children. Their evidence-safe Ranked runs away from the Relay entry now reject, and those runs were invulnerable. This is a runtime change, so the release needs its browser certification, including the Ranked e2e's HMH leg.

**The frozen v6 corpus.** With the twelve consistency flags removed, the 288 results still hash to the `60ea173a` digest. With them, thirteen results gain a flag and five change verdict: the three "combo above kills" cases reject `combo-above-kills`, and the two "caches x3" cases reject `pickups-above-capacity`. The other eight were already rejected (no ticks, or squeezed) and add what their time cannot hold. The new digest is `V6_CORPUS_DIGEST_1_8_4`.

**The honest corpus.** All 75 runs meet the twelve rejects (56 ok, 19 flagged, none rejected), most of them at the bound:
- in every run, the equipped ticks equal the ticks, the grenade kills equal the contacts, the detonations equal the throws plus launcher shots, and the damage total equals the weapons' sum;
- 14 runs throw exactly their hand-grenade supply, and in 5 runs the best combo is every kill;
- 121 effect rows sit exactly at the gated pickup capacity (89 of them effects with a site reward), across 170 site operations and 12 vault unlocks;
- one run equips 7 weapons.

**Residuals.**
- A consistent fabricated grenade trail still credits its grenade kills: a launcher cache, one launch and one blast of n contacts give n. Hard-fork-hero in 50 s stays open, because a 1.8.x blast has no target cap and the summary has no per-blast record.
- Damage has no time ceiling: a consistent `weapons[].damage` trail still earns damage-chain in a short run.
- A site milestone is a claim bounded only by the run's end tick. A summary that claims every site operated at tick 1 keeps nearly the full pickup capacity: about 250 in a 60-minute run, where an honest hoarder reaches 90 %. A travel bound per site would move that by a few hundred ticks.
- Flags do not withhold achievements (R0-6, an owner decision).
- A 1.8.x child cached by the service worker still honours `evidenceSafe` in Ranked until it updates.

### 16.7 Third round, item 1: the melee trail (`HMH-VERIFIER-R3-HANDOFF.md`)

`kills.byWeapon` for `litecoin-knife` and `forked-standard` reaches `statsFromHmhRunSummary` as `meleeKills` (blade-master at 100, blade-samurai at 250), and no rule read either weapon's trail: against `4f947386`, `hmh-valid` with 100 knife kills and no swing verified ok. Four consistency rejects close it (`HMH_V6_CONSISTENCY_RULES.melee`, `hmhV6MeleeCadenceLimit`), written only after 40 real headless runs of the 1.8.4 child in knife-heavy and Forked Standard play (`tests/fixtures/hmh-honest-corpus/real-child-1.8.4.json`, rows r68 to r107) showed what the child records. Each is an exact invariant of the 1.8.x child, pinned by `the melee-trail literals equal the 1.8.x child, and the knife swings only with a hit, a cooldown apart`. `HMH_V6_CONSISTENCY_REJECTS` now lists sixteen ids; with all sixteen removed the 288 results still hash to `60ea173a`, and `V6_CORPUS_DIGEST_1_8_4` is unchanged (no mutation touches a melee row). The stats-field inventory in the verifier header now names `meleeKills` beside the six fields of 16.6.

| Reject | Formula | Why every honest run passes |
|---|---|---|
| `knife-kills-above-contacts` | knife row `kills` > `projectileContacts` | `stepMeleeState` records one projectile contact per hit, and a kill is one of those hits (`combat-events` stamps the kill with the hit's weapon). Real corpus: 47 kills of 71 contacts at most |
| `melee-contacts-without-trigger` | a knife or Standard row with `projectileContacts` > 0 and `triggerContacts` = 0. Value: how many of the two rows | A swing that hits records its trigger contact in the same statement as its contacts (`main.mjs`, the knife trail and the `weapon:melee-strike` trail alike); the schema already keeps `triggerContacts` within `triggers` |
| `knife-triggers-above-cadence` | knife `triggers` > ⌈T / 20⌉ | The knife is stepped once a tick from tick 1, automatically only, and a swing sets the next one `HMH_MELEE_DEFINITION.cooldownTicks` (20) later. Real corpus: 12.3 % of the bound at most (the 1.8.3 brawler r08, 55 of 449) |
| `standard-triggers-above-cadence` | max(Standard row `triggers`, `forkedStandard.attacks`) > ⌈T / 18⌉ | The Standard is stepped once a tick while it is active, and a strike sets the next one a cooldown later (thrust 24, sweep 28, each less 2 per Attack Speed rank, three ranks at most: 18 at the least, plus a whiff penalty). Real corpus: 12.4 % at most (1.8.4 r96, 143 of 1,154) |

**Both corpora.** The scripted model (75 runs) never swings either weapon, so its verdicts are unchanged (56 ok, 19 flagged, none rejected). The real corpus (120 runs, 1,798,597 ticks of the 1.8.3 and 1.8.4 children) is 117 ok, 3 flagged, none rejected: `kills-near-capacity` on 1.8.3 r08 and 1.8.4 r70, `score-near-ceiling` on 1.8.3 r62. In every real run the knife's `triggers` equal its `triggerContacts`, the Standard's `triggers` equal `forkedStandard.attacks` and its `projectileContacts` equal `forkedStandard.contacts`, and each melee weapon's kills stay within its contacts (`tests/server-verify-hmh-real-corpus.test.mjs`, the melee-trail test).

**Residuals.**
- The Standard's kills against its contacts is the same impossibility as the knife's, and every real run supports it (24 kills of 38 contacts at most), but the committed `hmh-realistic` fixture credits 91 Standard kills with no attack and no contact and the scripted model credits Standard kills with no trail; both are outside round 3's scope, so that rule waits. The R3 hand-off gives the three steps to land it.
- A consistent fabricated knife trail (one hitting swing per claimed kill, inside the cadence) still verifies; the kill capacity, not the cadence, makes blade-samurai a 21,780-tick claim. The summary has no per-swing record, and a swing has no target cap.
