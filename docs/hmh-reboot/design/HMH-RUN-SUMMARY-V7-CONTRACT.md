...
| `BOSS_REINITIATION_MIN_TICKS` | 600 + 120 + 1,800 = **2,520** | A retreat ring appears only after 600 engaged ticks, the retreat channel takes at least 120 ticks, and the boss is ready again only 1,800 ticks after the retreat completes. So consecutive initiations of one boss are at least 2,520 apart, even if "engaged" counts the intro. A ring that counts its first armed tick fills at initiation + 719, so the child makes the boss ready again at the later of the retreat + 1,800 and the initiation + 2,520 (a trigger with no ring, like the Dark Pool, re-initiates on the first ready tick) |
...
- **Child progression release (build ledger slice 6).** The v7 fixtures now play the child's own offer API: `openRunUpgradeOffer`, `rerollRunUpgradeSlot`, and the upgrades and progression counters from `runUpgradeRows` and `runProgressionRow`. They were regenerated. `hmh-v7-four-bosses` keeps its guns dry (`armed: []`) and re-rolls for the Pistol's cards, so it still masters the Pistol; its Settler Rail now comes with the Foreman's Seal, and three Seals are still banked. The schema-6 fixtures stand for 1.8.x runs, so the builder keeps the 1.8.1 two-card offer frozen for them (`offer181`), and they rebuild unchanged.
