...
- **Visual:** same-seed census captures of the base and head builds at tick 1800 (mobile 1242×2688, i.e. 414×896 @3) align at shift 0 in all nine regions (no pivot drift), keep every actor on the same ground line, and differ only at actor edges and baked shadow rims (10.8% of pixels by more than 8 levels, 2.0% by more than 32); Laplacian sharpness drops 2.2% over the frame and about 9% in the actor-dense centre band, while terrain (lossless half tiles), HUD and touch controls are indistinguishable at 2× zoom. Desktop base and head differ by at most 3 levels on 0.67% of pixels with identical sharpness. `npm run visual:reboot` on the 2026-09-25 build of the same change: 12/12 scenes unchanged, no baseline update (frontier-relay-mobile meanDelta 0.424 of 2.5; enemy crops unchanged).
- **Frame timing:** none of this is visible to the CPU-throttled proxy, as expected on a host whose GPU is never the limit. Load-matched mobile pairs on the 2026-09-25 builds: 4x 18.33 → 18.58 ms mean (p95 27.7 / 27.7), 6x 37.96 → 38.45 and 37.68 → 39.16 ms (two later 6x pairs ran in windows where the unchanged base build itself took 72 ms, twice its earlier time, so they measure the host, not the build); SwiftShader 1x 54.55 → 54.19 and 54.47 → 54.97 ms; desktop stays vsync-bound (6.99 → 7.14 ms). The step's payoff on an A12 is memory pressure and bytes, not host frame time; owner-device acceptance remains open.
- **Provenance.** The census and the capture comparison were re-run on 2026-09-26 with the rebased builds (base = a `git archive` of `c34d2a31`, head = `HEAD`); the timing, SwiftShader, leak and `visual:reboot` figures above come from the 2026-09-25 runs on the pre-rebase step-5 and step-6 builds, whose HMH runtime path for the bench scene is the same as the rebased pair (the rebase only adds the Ranked-only evidence gate and the dormant-boss lightning guard to `main.mjs`). The rebased-build 4x/6x A/B and visual gate are queued behind the shared heavy lock (`.tmp/step6/job-timing.sh`, `job-visual.sh`); the evidence file marks them pending until they land.
