...
## Perf step 6: mobile texture pages and visual caps (2026-09-26)

Change (`3820bda2` half-resolution pages, `55f3786b` visual caps, `9df58d41` SwiftShader bench option, `dcf0a23e` hero and terrain test pins; branch `fable/hmh-perf-part3` on the rebased steps 3–5 at `c34d2a31`): the mobile profile used to upload the same 75 full-size actor, prop and terrain pages a desktop does, about 126 MB of RGBA textures behind a 414×896 canvas drawn at resolution 1, and it animated 32 bodies, drew all 48 blood marks and 16 atmosphere sprites, and baked its seven shadow, glow and fog textures through multisampled render buffers.

- **Half pages, same geometry.** `scripts/build-hmh-mobile-half-res.py` writes `<page>@0.5x.webp` beside each runtime page: the four hero pilot atlases and motion pages, the 30 held-weapon pages, the bagholder roster and the six native rosters (boss included), the authored-prop atlas, both Tripo prop pages, and the 11 terrain tiles, 11 fringes and 5 edge strips (48 atlases, 11 tiles, 16 strips; 15.0 MB against 73.0 MB of sources). `@0.5x` is Pixi 8's own resolution suffix, so the half page decodes with `source.resolution` 0.5 and keeps the full page's logical size; every frame rectangle, anchor, pivot, trim and UV in the unchanged full-size metadata maps onto it and nothing changes on-screen size or ground position. Resampling is premultiplied float32 Lanczos-3 at exactly 2:1, frame by frame for atlases (each frame is isolated on the 2:1 grid and only its own footprint texels are written, so the 2 px packing gutters cannot bleed; overlapping footprints are refused) and wrap-aware for tiles (U and V) and strips (U, clamped V). Atlases are lossy WebP q90 with lossless alpha; terrain is lossless exact WebP so tiles still wrap bit for bit. `--check` verifies source freshness, output bytes and per-frame alpha-centroid alignment: 75 pages, 0 problems, worst offset 0.328 source px (threshold 0.5). The manifest `apps/portal/assets/generated/hmh-mobile-half-res.json` pins the source and output SHA-256 of every page, so a regenerated source without a rebuilt variant fails the test suite.
- **Loader.** `runtime-performance.mjs` `profileTextureUrl()` rewrites a runtime page URL only on the mobile profile; `createProfileTextureLoader(Assets, profile)` returns Pixi's `Assets` untouched on desktop and reduced motion, and on mobile loads the variant and falls back to the full page if it fails. `main.mjs` routes every scalable page through it; the pilot-only mannequin, the silver coin and the world-design and barrier pages (which already carry mobile tiers) stay on `Assets.load`. Tripo page validation now reads the logical size, as the hero, motion, held-weapon and roster validators already did.
- **Visual caps, mobile only.** At most 24 animated enemy bodies (was 32); the newest 16 blood marks drawn (the pool still holds 48, the oldest kept mark paints first and unlanded marks never use the cap up); half the atmosphere budget (2 fog + 6 motes, was 4 + 12); and the contact-shadow, weapon-glow and atmosphere bakes take `antialias` from the profile, so phones skip the 1.76 MB of multisampled render buffers behind the seven bakes (resolution stays 2, so the gradients are unchanged). Desktop and reduced motion keep their values. Every cap decides only how much art is drawn; none is read by the simulation.
- **Audited and left alone.** Roster atlases already load lazily per archetype (with warm-ups for the opening pair, `liquidator-agent` after the start gate, the pressure trio at tick 16,200 and the boss 600 ticks before it arrives; the census confirms the boss roster is never fetched in the crowd scene). Pixi 8.19's `GCSystem` already releases GPU textures idle for 60 s. Decals are baked since step 2 and cost nothing per frame, and contact shadows are about 0.1 ms of a 25 ms busy frame; a cap on either would pop art in and out.
- **Tooling.** `scripts/hmh-perf-crowd-bench.mjs --gpu=swiftshader` renders WebGL through ANGLE's CPU rasterizer so texture size and fill cost can reach an A/B; absolute times there mean nothing.

Exactness:

- The headless digest is unchanged (`2488a609…`; crowd `50f3f612…`, director `7f38bb3e…`, endurance soak `2e38a09a`), the browser census trace at tick 1800 is `449616fb…` with full and with half pages on mobile and desktop, and the determinism, run-summary, evidence-schema and touched-module suites pass (84 tests).
- `tests/hmh-mobile-half-res-textures.test.mjs` (10 tests): all 75 runtime pages switch to their variant on mobile and to nothing on desktop or reduced motion; tiered pages and non-page URLs are never rewritten; Pixi reads the suffix as resolution 0.5, query strings included; the manifest lists exactly the runtime pages, each fresh, half size and byte-exact; the loader falls back to the full page; roster, hero (pilot + motion + held pages, 576 poses per hero), Tripo and terrain displays cut from half pages draw the same size, anchor, scale, frame, UVs and texture matrix as from full pages. `tests/hmh-mobile-visual-caps.test.mjs` (5 tests) pins the caps, the paint order of the kept marks, the halved atmosphere budget and the MSAA-free bakes.
- Initial JS is 1,045,282 B, 697 B more than the rebased step-5 build (3,294 B of headroom).

Evidence: `docs/testing/hmh-perf-step6-mobile-assets.json`.

- **Texture memory and bytes (bench census, tick 1800, mobile 414×896 @3; rebased builds `c34d2a31` → `HEAD`, 2026-09-26):** GPU textures 125,794,060 → 35,256,076 B (120.0 → 33.6 MiB, −72%), the 1.76 MB of MSAA render buffers → 0, fetched image files 25.3 → 5.4 MiB (−79%; 38 half pages in place of 38 full ones), decoded RGBA 177.5 → 52.4 MB, 23 textures either way. Desktop is byte-identical base and head (171,581,884 B, 26.7 MiB fetched). The 2026-09-25 pre-rebase census gave the same numbers to the byte. Over a 21,600-tick run the renderer process holds about 100 MB less private memory at every round (273→322 MB instead of 372→432 MB) and the GPU process 275–419 MB instead of 430–482 MB; the JS heap is identical.
- **Visual:** same-seed census captures of the base and head builds at tick 1800 (mobile 1242×2688, i.e. 414×896 @3) align at shift 0 in all nine regions (no pivot drift), keep every actor on the same ground line, and differ only at actor edges and baked shadow rims (10.8% of pixels by more than 8 levels, 2.0% by more than 32); Laplacian sharpness drops 2.2% over the frame and about 9% in the actor-dense centre band, while terrain (lossless half tiles), HUD and touch controls are indistinguishable at 2× zoom. Desktop base and head differ by at most 3 levels on 0.67% of pixels with identical sharpness. `npm run visual:reboot` on the rebased head build (and on the 2026-09-25 build): 12/12 scenes unchanged, no baseline update (frontier-relay-mobile meanDelta 0.424 of 2.5, desktop scenes 0.01–0.231; enemy crops unchanged).
- **Frame timing:** none of this is visible to the CPU-throttled proxy, as expected on a host whose GPU is never the limit. On the rebased builds (`c34d2a31` → `HEAD`, twelve alternating pairs on 2026-09-26) other sessions kept the host at 19–76% other load and no pair is strictly load-matched; the pairs within 5 points agree with the 2026-09-25 matched pairs: 4x 17.25 → 18.31 ms (p95 27.7 / 27.8, p99 34.6 / 34.8) and 21.72 → 21.35 ms (p95 34.7 / 34.7), 6x 38.07 → 37.91 ms (p95 55.6 / 55.6, p99 69.4 / 69.4); every other pair moves with the load difference, and the SwiftShader pairs at 37–50% load alternate sign. 2026-09-25, load-matched: 4x 18.33 → 18.58 ms (p95 27.7 / 27.7), 6x 37.96 → 38.45 and 37.68 → 39.16 ms (two later 6x pairs ran in windows where the unchanged base build itself took 72 ms, twice its earlier time, so they measure the host, not the build); SwiftShader 1x 54.55 → 54.19 and 54.47 → 54.97 ms; desktop stays vsync-bound (6.99 → 7.14 ms). The step's payoff on an A12 is memory pressure and bytes, not host frame time; owner-device acceptance remains open.
- **Provenance.** The census, the capture comparison, the 4x/6x and SwiftShader A/B and `visual:reboot` were run on 2026-09-26 with the rebased builds (base = a `git archive` of `c34d2a31`, head = `HEAD`, bench serving `apps/portal` itself with `--affinity=0xFFFF`); the leak-probe figures and the load-matched timing pairs come from the 2026-09-25 runs on the pre-rebase step-5 and step-6 builds, whose HMH runtime path for the bench scene is the same as the rebased pair (the rebase only adds the Ranked-only evidence gate and the dormant-boss lightning guard to `main.mjs`). Both sets are in the evidence file, each labelled.
