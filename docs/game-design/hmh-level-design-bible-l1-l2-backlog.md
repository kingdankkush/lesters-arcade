...
|---|---|---|---|
| 1 | 🟡 PARTIAL | **Combat core**: pooled projectiles + swept collision + muzzle/impact FX; then grenades (arc / bounce / fuse / AoE + **landing shadow marker showing blast radius**) | Bullets (`updateBullets`/`updateRoguelikeBullets`), `combat.bullets[]` array, `spawnMuzzleFlash`, impact sparks all live in `main.js`. Swept-AABB + circle overlap + `stepProjectile` (gravity/bounce) are pure & tested in `combat-physics.mjs`. **Gaps:** projectiles use a plain array (re-alloc each shot), not a recycled pool; grenades are an **instant blast at the aim point** — no in-flight arc, no fuse, and **no landing shadow / blast-radius telegraph** (the bible's explicit readability ask in §6.3). First slice target. |
