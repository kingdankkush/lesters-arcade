...
- **History is checked, not trusted.** Derivation throws unless the history has the same `gameId`, `runs` as a non-negative integer, `unlockedIds` as an array of strings, and every `historyFieldsFor` path as a finite number in `sums` and `maxima`. A missing or string value would otherwise read as "nothing yet": held ids would come back, and run-count and cumulative entries would silently never unlock. The index query must cast `count(*)` and `sum(...)` to numbers, because the Neon driver returns them as strings.
...
| 43 | `speed-clear` | Speed Clear | diamond | skill | Intended: Beat the Level 1 boss in under 8 minutes. | **no**: in schema 6 the boss band opens at tick 72,000 (20 minutes) and the summary has no boss-kill tick. Schema 7 records the kill tick (`bosses.liquidator.defeatedTick`), but the Liquidator is ready only at tick 36,000 (10 minutes), so a kill under 8 minutes is still impossible; the id would need a new threshold before it could unlock. | no | Unavailable |
