...
Those exports keep the reference analysis connected to district-generation context, tests, and the PixelLab production queue instead of living only as prose.

## 6. Current P0 queue

The next practical production step is now generated by:

```bash
npm run assets:hmh:level1:queue
```

That writes `scripts/pixellab-hmh-level-one-environment-queue.json` with 18 P0 `create_map_object` jobs for:

- ground textures
- roads and paths
- water and shorelines

The queue intentionally uses PixelLab's flat MCP argument shape (`description`, `width`, `height`, `view`, `outline`, `shading`) and excludes nested `image_size`, which the PixelLab API rejects. Treat every output as a candidate until it passes palette, outline, alpha, atlas, and contact-sheet QC.
