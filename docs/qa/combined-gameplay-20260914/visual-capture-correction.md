...
A later cold-start repeat exposed a second race: the artwork-ready frame could contain zero or one simulation step before the mutation observer restored aim. The final visual fixture uses a controlled animation clock: 1 ms startup frames, a hold at artwork readiness until the fixture is armed, then 17 ms presentation frames. This fixes the input boundary before the first canonical step. The 60 Hz simulation and four-step catch-up limit are unchanged. The controlled-clock run is strictly visual evidence; real-clock touch, portal and performance checks remain separate.
