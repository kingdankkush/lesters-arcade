...
`scripts/rehearse-step7-dry-run.mjs` flips the flags in a throwaway git worktree under the OS temp
directory (never committed, merged or pushed; removed at the end), regenerates the address module from a
local deploy and the public copy, runs every step of `npm run vercel:build` plus the offline design
audits, separates the failures the flip causes from ones already present, and proves the edits below.
The raw evidence is `docs/qa/step7-dry-run-20260923.json`.

<!-- step7-checklist:start (generated by scripts/rehearse-step7-dry-run.mjs; do not edit by hand) -->

Generated from `docs/qa/step7-dry-run-20260923.json` (dry run at `b1732a4b`, 2026-09-23). Runbook step 7 (contract §13) makes exactly these changes in ONE commit, after step 6 and before the 1.8.0 build:

1. **Address module.** Commit `contracts/deployment-record.hardened.json` from the step-3 broadcast and regenerate `apps/portal/src/generated/litvm-addresses.mjs` with `npm run contracts:addresses` (`status: 'deployed'`, `startBlock` from the record). The §9.1 invariant test fails the build while the module is still `predicted`.
2. **Flags** in `apps/portal/src/settlement.mjs`:
...
3. **Public copy.** Run `node scripts/build-portal-pages.mjs` (contract A33) and commit what it rewrites. In the dry run the regeneration changed:
...
4. **Pinned tests** (0 edits, each proven by the dry run):
   - none curated yet
   - **NOT COVERED** (this dry run did not pass; 16 failures caused by the flip still fail):
     - `tests/hmh-release-facts.test.mjs:170 :: real repository adapters cover all 55 groups, 67 feature IDs and all 99 original namespaced IDs`
     - `tests/litvm-addresses-module.test.mjs:84 :: committed address module is generated and matches either the §8.1 prediction or the hardened record`
     - `tests/litvm-ranked-contract-gate.test.mjs:68 :: configured read-only GameRegistry address exactly matches the generated address module`
     - `tests/litvm-ranked-contract-gate.test.mjs:100 :: approved current-ABI fixture passes only contract/funds preflight at one pinned block`
     - `tests/litvm-ranked-contract-gate.test.mjs:145 :: the direct ranked writer refuses disabled settlement before any provider access`
     - `tests/litvm-ranked-contract-gate.test.mjs:163 :: a source-isolated live writer rechecks approval before creating any signer`
     - `tests/name-claim-prompt.test.mjs:175 :: the toast is skipped until the contracts are deployed`
     - `tests/ranked-client-wiring.test.mjs:60 :: settlement identity is rankedIdentityFor(session): the per-game key the entry paid`
     - `tests/ranked-client-wiring.test.mjs:148 :: live HMH settles through the relayed client with a body the server verifies`
     - `tests/ranked-client-wiring.test.mjs:194 :: live Chikun settles the verified v6 evidence and publication stamps the local rows`
     - `tests/ranked-client-wiring.test.mjs:388 :: an entry that failed on chain makes the run practice and nothing is posted`
     - `tests/ranked-client-wiring.test.mjs:415 :: a lazy chunk that fails at game over is fetched again, and a paid run is never dropped silently`
     - `tests/ranked-preflight.test.mjs:173 :: reads go through the public RPC, not the wallet`
     - `tests/ranked-preflight.test.mjs:274 :: sendRankedEntry resolves on broadcast and reports confirmation later`
     - `tests/settlement.test.mjs:16 :: ranked settlement fails closed until the verifier contract and attestation service are approved`
     - `tests/stacked-ranked-settlement.test.mjs:61 :: persistRanked writes the replay store with the evidence digest and builds the settle body`
5. **Audits and build steps.** No step of `npm run vercel:build` and no offline design audit fails because of the flip.
6. **Already failing at the dry-run base** (not step-7 work; they fail without the flip too):
   - test `tests/portal-trust-copy.test.mjs :: main.js purges House Demo rows on load and gates every index read on HOSTED_PROFILE_SYNC`
   - test `tests/ranked-results.test.mjs :: main.js mounts the screen from exactly one lazy listener`
   - test `tests/wallet-session.test.mjs :: a token stored by the 1.7.0 flow is discarded instead of restored or sent`
   - `node scripts/hmh-release-facts.mjs --check` exits 1 at the base as well (Stale or missing fact sheet: hmh-fact-sheet.json)
7. **Gate.** `npm run vercel:build` then shows exactly the 51 ledgered failures plus the 3 listed in item 6 until their owner fixes them. Restore `docs/testing/hmh-reboot-test-retirement-gate.json` before committing unless the orchestrator is committing the integration's gate JSON.
