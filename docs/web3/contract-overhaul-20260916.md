...
Generated from `docs/qa/step7-dry-run-20260923.json` (dry run at `d23818a3`, 2026-09-23). Runbook step 7 (contract §13) makes exactly these changes in ONE commit, after step 6 and before the 1.8.0 build:
...
4. **Pinned tests** (11 edits, each proven by the dry run):
   ...
   - `tests/ranked-preflight.test.mjs:272` (the committed entry is live after step 7; the disabled refusal is kept on the isolated module):
     ...
     ```
   - `tests/name-claim-prompt.test.mjs:174` (the default reads the generated module, which step 3 made deployed):
     ```diff
...
7. **Gate.** `npm run vercel:build` then shows exactly the 51 ledgered failures plus the 3 listed in item 6 until their owner fixes them (the dry run's full re-run after the edits: FAIL, 3 unexpected, 0 missing). Restore `docs/testing/hmh-reboot-test-retirement-gate.json` before committing unless the orchestrator is committing the integration's gate JSON.
