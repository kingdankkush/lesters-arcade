...
| --- | --- | --- |
| `node scripts/rehearse-jackpot-week.mjs` | Runs R0 (the soft-launch week) and R1-R20 (plus R3b, R12d and R13v) on one local stack, each scenario in its own weeks, and writes `docs/qa/jackpot-rehearsal-<YYYYMMDD>.json`. `--only R1,R13` runs a subset, `--no-write` skips the receipt, `--out <path>` writes elsewhere, `--json` prints it. Exit 1 when a scenario fails. Commit the scripts first: a receipt to commit comes from a clean tree (`dirty: false`). | about 1 minute |
| `node --test tests/jackpot-rehearsal.test.mjs` | The driver checks and the fast subset R0, R1, R3a, R5, R10, R13, R15 and R17 (it also runs inside `npm test` and the release gate), plus the committed receipt and checklist checks. | about 25 s |
| `node --test tests/jackpot-api-contract.test.mjs` | The real `/api/jackpot` at every lifecycle stage through the UI's `parseJackpot` and surfaces (each stage against its `tests/fixtures/jackpot/*.json` fixture), and the real review API against the owner page's fixture and through the owner model's timeline geometry. | about 10 s |
| `node --test tests/jackpot-live-dry-run.test.mjs` | The live dry run against a local HTTP server and the in-process chain (read-only proof, every check, and the failing case of each: J17 drift, pauses, a stale cron, the admin backlog, a low keeper, an unverified role history, an unfunded week, the previous week after a migration). | about 25 s |
| `node scripts/jackpot-flag-flip-dry-run.mjs --confirm-throwaway` | The E10 commit rehearsed in a throwaway worktree made from HEAD (E4 first, then the flip, the whole suite at each state, then every pinned assertion of each broken test); writes `docs/qa/jackpot-flag-flip-checklist.json`. Refuses the main worktree. Commit first: the throwaway never sees uncommitted changes (`dirty`). | about 35 minutes |
| `node scripts/jackpot-live-dry-run.mjs --site https://lestersarcade.io [--expect-live false\|true] [--require-funded] [--expect-paused true\|false] [--logs-from <block>] [--funder <address> ...]` | The read-only live checklist (below). The owner session only; exit 1 when a check fails. | seconds |
...
- `ok`, `summary` (scenarios, passed, failed ids, expected misses, routed design findings) and `stack` (the local addresses, the launch rules, the E5 list).
- Provenance: `head` and `dirty` (the commit the run used, and whether tracked files differed from it), `inputs` (the sha256 of the script, the driver and `scripts/lib/local-jackpot.mjs`; `tests/jackpot-rehearsal.test.mjs` refuses a receipt when any of them changed, or when `dirty` is true) and `productSha256` over `productFiles` (the contract, the jackpot API, cron, server and ops modules and the UI parser the run proved). The product digest is recorded, not enforced: after any change to those files, re-run the script and commit the new receipt.
...
- `designFindings`: couplings the design or another owner must take up (`status: 'routed'`, `routedTo`, and `fixedHere` for what this slice fixed). Not failures; the owners named decide.

What each scenario proves:

| Id | Proves |
| --- | --- |
| R0 | The soft-launch week (runbook E9: the first epoch, `adminClearOnly = true`): the keeper selects, screens and submits but creates no clear (a keeper `clear` reverts `REVIEW_LOCKED`); at C + 24 h the week is `awaiting-admin` and `finalize` reverts `LEADER_NOT_CLEARED`; the admin clears the top eligible run from the owner page's encoding and it is paid |
...
| R12d | A dust week (0 < pot < `minFundWei`: 100 tCHIKUN rolled into a week whose rules ask for 150) with an eligible run: `unfunded`, no keeper transaction, no amount in the API, the UI or the Chikun child, no champion (funded means `total >= minFundWei`, never `> 0`) |
| R13 / R13v | Five decoy wallets displace the honest top 5; the keeper flags them; the admin disqualifies them (`multi-wallet`, whole wallet); the keeper re-lists the displaced honest rows within a few runs, before payout − 2 h (the ones after C + 12 h through the `wasListed` exception), and the chain refuses a re-list at payout − 2 h; the honest leader is paid. R13v: the literal C + 11 h 59 min timing with a screening backlog (the decoys, on chain, take all five screens of the first run while five re-screened keeper rows wait), the re-list after that public displacement, and the never-listed honest #6 added with `adminSubmit` and paid |
...
| R17 | Stale keeper actions never override the admin: the keeper's flag of the leader is dropped; the owner re-screens the leader before any admin decision; in the next run the screen re-holds it and the admin's clear lands mid-run, after the index and the screen (the race the pre-send check exists for). Action ids are per week, kind and session, so the re-held flag and the dropped one are one row: re-signed, it ends `skipped`/`review-locked` by the pre-send check and never reaches the chain; a later rescreen is refused; a keeper `flag` reverts `REVIEW_LOCKED`; the leader is paid |
| R18 | A compromised admin: `operator-pause` holds against its `unpause`; its `transferAdmin` reverts `OPERATOR_LOCK` while it is admin, and `ONLY_ADMIN` after `force-admin` (one transaction); the review API still answers the old admin from its 60 s cache and refuses it once that expires; the new admin reinstates and clears; the operator unpauses |
...
| R20 | Ranked-setting drift: with the reserve cut to 0 (the settle floor unchanged) the live checker is red at once (`j17-settle-floor`) and a player who pays the quote is refused at settle (402 `entry-underpaid`); the same Monday step lowering `RANKED_MIN_PAID_WEI` settles it, keeps it eligible and turns the checker green; a repointed `rankedEntry` and fees off fail the live checker; the entry-modal row hides below `minPaidWei`. Every setting is restored in a `finally` |

Findings the rehearsal recorded (none is a product bug; each is in the receipt's `notes`, and the routed one in `designFindings`):
...
- **The settle floor is part of J17 (R20, routed).** A reserve cut to 0 keeps runs jackpot-eligible (`minPaidWei` is the flat 0.1 fee), but the server refuses entries paid 0.1 zkLTC with `402 entry-underpaid` until `RANKED_MIN_PAID_WEI` is lowered to match (default 0.102), and nothing refunds the entry. `/api/health` now reports the floor (`settleMinPaidWei`) and the live dry run fails `j17-settle-floor` while the quote is below it, so the weekly check sees the drift before players pay. Routed to the design owner (list it under §A.5 J17) and the Ranked settle owner (derive the floor from the quote, or refuse at entry).
...
`docs/qa/jackpot-flag-flip-checklist.json` (schema `lesters-jackpot-flag-flip-checklist-v1`): `head` and `dirty` (made from a clean committed tree), `e4` (what runbook E4 itself changes in the tree and which tests the E4 commit updates), `flip` (the two edits, the build, every file the flip changes with the changed literals in `literalsChanged`, and `testsToUpdate`: each test the flip breaks with every assertion it stops at in `assertions` (line, source, expected and actual, found by re-running it with each failing assertion neutralised in the throwaway copy) and `complete`; a failure that passes when its file runs alone is a load flake, set aside in `flakes`), and `checklist` (files to edit, commands, preconditions, the commit, the release, the post-release smokes and the rollback). Treat `testsToUpdate` as at least these: a test the flip first breaks at one line can hold more pins, which is why each is re-run until it passes.
...
`rpc-chain`, `jackpot-deployed`, `code-jackpot`, `code-token`, `code-retired`, `immutables`, `roles` (against `contracts/deployment-record.jackpot.json`), `api-live`, `week-key` (`weekOf(now)` against the server's week), `rules-in-force` (`minFundWei > 0`), `j17-quote` (`quoteEntry(chikun).totalWei >= minPaidWei`), `j17-settle-floor` (the quote `>=` the server's settle floor, `/api/health` `settleMinPaidWei`), `j17-ranked-entry`, `j17-season`, `j17-game-id`, `j17-game-registered`, `pot-current`, `pot-previous` (chain against `/api/jackpot`; the previous week from the instance the API names, the retired one after a migration), `pot-funded` (`potOf(currentWeek).total >= minFundWei` on chain: a failure with `--require-funded`, a reported fact without), `pause` (no pause of any kind, env, admin or operator, and `/api/health` agreeing with the chain; `--expect-paused true` while an emergency stop is intended), `keeper-balance` (≥ 0.05 zkLTC), `cron-fresh` (20 min, and not skipping under `JACKPOT_PAUSED`), `admin-backlog` (no week awaiting the admin over 24 h or of unknown age, none claim-pending), `no-failed-weeks`, `block-list` (the test wallet `0x8841…ce824`, the verifier, the relayer, each `--funder`), `residual-recipient` (blocked or staff), `staff-ever` (admin, operator, keeper), `staff-history` (every address the role events named, from the instance's `startBlock` or from `--logs-from`), `jackpot-live-flag` (the served `JACKPOT_LIVE` and the rules page's `noindex` against `--expect-live`, and the served `SITE_VERSION` equal to the one `/api/health` reports).

No check passes by being skipped. `staff-history` reads the role events in at most 60 `eth_getLogs` calls of 10,000 blocks; LiteForge makes about 4 blocks a second, so a range from `startBlock` outgrows that within two days and the check then fails as `unverified`. Pass `--logs-from <scannedTo + 1>` from the previous run's detail (at E9, then every E11 run): `staffEver` is never unset, so what an earlier run verified stays verified. `jackpot-live-flag` reads `/src/jackpot-config.mjs`, a proxy for the bundle (the esbuild output in `dist/` inlines and minifies the literal away); it holds because Vercel builds `dist/` from the same `apps/portal` tree in the same deployment, which the version comparison ties to the running release.
...
| E1 | Commit, then `node scripts/rehearse-jackpot-week.mjs` from the clean tree → commit `docs/qa/jackpot-rehearsal-<date>.json` (R0-R20 passed; R16 as the expected miss; `designFindings` read and routed). |
...
| E9 | The soft-launch week (flag false, `adminClearOnly = true`, rehearsed as R0): run `node scripts/jackpot-live-dry-run.mjs --site https://lestersarcade.io --require-funded` during the week, then at C + 2 h (selection), C + 12 h (window closed) and C + 24 h (payout; `--require-funded` then concerns the new week); keep the last run's `staff-history` `scannedTo` for `--logs-from`. The keeper never clears this week: at C + 24 h the week is `awaiting-admin`, and the admin clears the top eligible run under the rubric (never disqualify to steer the winner). Receipt `docs/qa/jackpot-soft-launch-<date>.json`. |
| E10 ⚠ | Follow `docs/qa/jackpot-flag-flip-checklist.json`: check every `checklist.preconditions` entry (the funded week with `node scripts/jackpot-live-dry-run.mjs --site https://lestersarcade.io --require-funded`), make the two edits (`JACKPOT_LIVE = true`; remove the `LEGAL-REVIEW-PENDING` comment with the confirmed legal text in place), run `node scripts/build-portal-pages.mjs` and `npm run assets:hmh:curated-level-kit-runtime`, update at least every assertion listed in `flip.testsToUpdate` (then run those test files: nothing else may fail), commit, `npm run test:release` (exactly 51), release, then the `checklist.postReleaseSmokes` and `node scripts/jackpot-live-dry-run.mjs --site https://lestersarcade.io --expect-live true --require-funded`. |
| E11 | Weekly: funders fund the next week before Monday (at most one or two weeks ahead); `node scripts/jackpot-live-dry-run.mjs --site https://lestersarcade.io --expect-live true --require-funded --logs-from <last scannedTo + 1>` every Monday and before and after any Ranked setting change (below); review any `awaiting-admin` week within 24 h under the rubric (the 7-day default applies after that); contact a `claim-pending` winner well before the 180-day recycle; keep the keeper above 0.05 zkLTC. |
...
- the entry fee, the settlement reserve, fees on or off: `j17-quote` needs `quoteEntry(chikun).totalWei >= minPaidWei` of the new week. **A reserve change also changes what the server accepts**: `j17-settle-floor` needs the quote `>=` `RANKED_MIN_PAID_WEI` (reported by `/api/health` as `settleMinPaidWei`); lower it in the same step (R20), or new 0.1 entries are refused with `entry-underpaid` and are not refunded;
...
While a stop below is intended, run the live dry run with `--expect-paused true` (its `pause` check fails on any pause otherwise, and `cron-fresh` on the env pause); drop the flag once the stop is lifted.
