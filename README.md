# Lester's Arcade — 1.6.0 candidate: weapon wheel, sharing, Web3 groundwork (built, awaiting promotion)
...
Candidate deployment `dpl_AkmKkzZKqr7scPMDe5ACU6J7tKVT` (https://lesters-arcade-didds3ryr-justin-agent-projects.vercel.app) is READY from source `81a03ef6` on `fable/master-list-20260916`; site/game version `1.6.0`; cache marker `lesters-arcade-v51-weapon-wheel`. The local gate passes 3,791 of 3,842 tests with exactly 51 unchanged retired exceptions, and the hosted build ran the same gate. It has **not** been promoted: the promote command was blocked in the authoring session, so https://lestersarcade.io still serves the previous section's deployment (`dpl_Gnf2tetZUqK5fsfH7Pgcy4gERe7B`, the rollback target). Promote with `npx vercel promote dpl_AkmKkzZKqr7scPMDe5ACU6J7tKVT --yes`, then verify public hashes. [Release receipt](docs/qa/master-list-release-20260916.json).
