...
export function createStackedBoardView({ index, cells = 10, rows = 24, frame, geometry, Container, Graphics, Text, onFrameReleased = () => {}, onPresent = () => {} }) {
  ...
  // Unlockable piece skin (contract §7.9): a palette from cosmetic-palettes.mjs, or the classic colours.
  let colors = COLORS;
  const drawWellFrame = () => {
  ...
  const renderPreview = (piece, originX, originY, scale=0.55) => {
    const normalized=normalizePiece(piece);
    let used=0; for (const [x,y] of pieceCells(normalized)) { const visual=previewPool.acquire(layers.hudLayer); drawMino(visual,{ x:originX+x*CELL_PX*scale,y:originY-y*CELL_PX*scale,kind:normalized.kind,size:(CELL_PX-2)*scale,patterned:colorblindPieces,colors }); used++; } return used;
  };
  ...
  const setModel = model => {
    ...
    label.text=`SCORE ${(model.score??0).toLocaleString()}\nLEVEL ${model.level??model.startLevel??1}   LINES ${model.lines??0}\nHASHPOWER ${model.hashpower??0}/8`; label.position.set(currentFrame==='wide'?408:8,currentFrame==='wide'?510:650);
    for (const [id, text, x, y] of [['hold-label', 'HOLD', 8, currentFrame === 'wide' ? 24 : 730], ['next-label', 'NEXT', currentFrame === 'wide' ? 416 : 140, currentFrame === 'wide' ? 24 : 730]]) {
      let heading = layers.hudLayer.children.find(child => child.__heading === id);
      if (!heading) { heading = new Text({ text, style: { fill: '#9ebcc9', fontFamily: 'system-ui, sans-serif', fontSize: 15, fontWeight: '600' } }); heading.__heading = id; layers.hudLayer.addChild(heading); }
      heading.position.set(x, y);
    }
    previewPool.end();
    trailPool.begin(); let trails=0;
    if (trailsEnabled && model.active) for (let step=1;step<=2;step++) for (const [x,y] of pieceCells(model.active)) { const point=boardCellToAuthored({x,y:y+step,frame:currentFrame}); if(point.visible){ drawMino(trailPool.acquire(layers.effectLayer),{...point,kind:model.active.kind,alpha:0.06,size:CELL_PX-5,colors}); trails++; } }
