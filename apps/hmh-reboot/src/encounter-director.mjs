...
import { encounterEnemyHealth } from './opening-balance.mjs';
...
export function stepEncounterDirector({
  ...
  const insertedEnemy = population.active.find((enemy) => enemy.id === result.enemyId);
  insertedEnemy.health = insertedEnemy.maxHealth = encounterEnemyHealth(result.archetypeId, insertedEnemy.maxHealth, tick);
