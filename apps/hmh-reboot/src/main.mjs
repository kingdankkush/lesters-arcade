...
async function boot() {
  ...
  // Projection only: the per-frame body pass (poses, transforms, shadows,
  // elite rings, hit reactions, health pips, attack tells), a lazy chunk
  // fetched beside the renderer init like the pool. See enemy-render-pass.mjs.
  const enemyRenderPass = (await enemyRenderPassModule).createEnemyRenderPass({
    markers: enemyMarkers,
    facing: enemyVisualFacing,
    hitFeedbackById: enemyHitFeedbackById,
  ...
  const syncEnemyMarkers = (enemies, rebuild) => {
    enemyDisplayPool.sync(enemies, rebuild);
    enemyRenderPass.pruneFacing();
  };
  ...
  const renderWorld = (renderState = renderActor ?? actor) => {
    ...
    if (renderState && camera) {
      ...
      // Every enemy body, attack tell and health pip for this frame, drawn
      // without a per-enemy allocation (enemy-render-pass.mjs).
      const animatedEnemyCount = enemyRenderPass.render({
        enemies: grayboxEnemies,
        camera,
        view,
        tick: simulation?.tick ?? 0,
        heroScreen: screen,
        cullMargin: performanceProfile.enemyCullMargin,
        animationBudget: Math.min(performanceProfile.maxAnimatedEnemies, encounterAnimationCap),
        simulationActive: Boolean(simulation),
        contactShadowPool,
        hitFeedback: enemyHitFeedback,
        particleScale,
        reduceMotion: settings.reduceMotion || performanceProfile.particlesPerHazard === 0,
