...
// Fit the authored frame, including viewports smaller than it. A minimum scale
// of one clipped small screens; integer snapping also wasted large desktop space.
const integerScale = value => Math.max(0.001, value);
...
export function layoutMatch({ viewportWidth, viewportHeight, boardCount, opponentMini = false, safeArea = {} }) {
  ...
  let scale;
  if (portrait) scale = integerScale(Math.min(usableHeight * 0.94 / size.height, usableWidth * 0.94 / size.width));
