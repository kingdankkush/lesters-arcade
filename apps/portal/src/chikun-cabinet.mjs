...
export const CHIKUN_CABINET_VERSION = '0.3.0';
export const CHIKUN_RUNTIME_VERSION = 'canvas-runtime-v1';
...
function forkGeometry(seed, index, tick) {
  ...
  const gapHalf = hazard.safeGapHeight / 2;
  const gapCenter = gapHalf + 38 + deterministicRoll(seed, index, 7) * (cfg.canvas.height - (gapHalf + 38) * 2);
  const x = cfg.canvas.width + hazard.firstOffsetPixels + index * hazard.spacingPixels - cfg.rules.scrollPixelsPerTick * tick;
  return {
    id: `fork-${index}`,
    index,
    x,
...
function relevantForks(seed, tick) {
  const cfg = CHIKUN_VERTICAL_SLICE_CONFIG;
  const hazard = cfg.hazards[0];
  const traveled = cfg.rules.scrollPixelsPerTick * tick;
