// E4 GET /api/settle/status → /api/settle-status — FAIL-CLOSED STUB.
//
...
// pure settleStatusRequest export, buildDeps and createHandler (§10.4 rule 5, A30).
...
export async function settleStatusRequest() {
  return { status: 503, body: { ok: false, error: 'not-implemented' }, headers: { 'Cache-Control': 'no-store' } };
}

const adapter = makeHandler({ label: 'settle-status', methods: null, query: null, run: settleStatusRequest });
