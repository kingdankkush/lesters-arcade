...
test('the live render states the launch copy and drops the preview copy', () => {
  withTempDir(dir => {
    ...
    assert.match(launch.description, /0\.012 testnet zkLTC per run, with verified scores published on LitVM/);
    ...
    assert.match(index, /0\.012 zkLTC per run on the LitVM LiteForge testnet: a 0\.01 zkLTC entry, split 85% to the game&#39;s developer and 15% to the arcade, plus a 0\.002 zkLTC settlement reserve/);
    ...
    assert.match(block(trust, 'ranked-status'), /Ranked is live on the LitVM LiteForge testnet\. A Ranked run costs 0\.012 zkLTC/);
...
test('the preview render keeps the preview wording and names no board period', () => {
  withTempDir(dir => {
    ...
    // verbatim (jackpot design §F.1) names the 0.012 Ranked price. The preview banner pays nothing.
...
test('the CLI writes an overridden render only to --out and leaves the committed pages alone', () => {
  withTempDir(dir => {
    ...
    for (const game of PORTAL_GAMES) assert.match(readFileSync(join(dir, `discover/${game.slug}.html`), 'utf8'), /0\.012 testnet zkLTC per run/);
  });
});
