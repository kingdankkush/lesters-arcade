...
test('boss phase uses its original local PCM sample through the boss-family cue policy', () => {
  const audio = fresh();
  const first = audio.play('boss-phase', { now: 1_200, volume: 0.14 });
  const blocked = audio.play('boss-phase', { now: 1_201, volume: 0.14 });
  assert.equal(first.played, true);
  assert.equal(blocked.reason, 'cooldown');
  assert.equal(FakeAudio.instances.length, 1, 'the cooldown must not allocate another voice');
  assert.match(FakeAudio.instances[0].src, /^\.\.\/assets\/audio\/sfx\/hmh-boss-phase\.wav$/);
