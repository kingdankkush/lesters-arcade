...
test('the cron migrates first and reports the schema version', async () => {
  ...
  try {
    ...
    assert.deepEqual(response.body.appliedMigrations, [1], 'the unmigrated database was migrated by the cron');
    assert.deepEqual(
    ...
    assert.deepEqual([again.body.schemaVersion, again.body.appliedMigrations], [1, []]);
