...
test('Cycle 007 prop manifest covers weapons, P0 pickups, power-ups and world dressing', async () => {
  ...
  assert.deepEqual(byCategory.weapon.map((asset) => asset.assetId).sort(), ['auto-miner', 'coin-blaster', 'hash-rail', 'launcher-rig', 'lightning-ledger', 'scatter-shotgun']);
  assert.deepEqual(byCategory.pickup.map((asset) => asset.assetId).sort(), ['berserk-candle', 'bonus-life', 'hash-rail-core', 'lightning-ledger-cache', 'nuke-liquidation', 'time-dilation']);
