...
test('the panel ships lazily, builds DOM safely, reads at 320 px and keeps phase 1 copy', () => {
  ...
  assert.match(main, /setCharacterUnlockOptionsProvider\(\(profile\) => \(connectedWallet && String\(profile\?\.wallet \?\? ''\)\.toLowerCase\(\) === String\(connectedWallet\)\.toLowerCase\(\) \? characterUnlockOptions\(\) : \{\}\), \{ hosted: HOSTED_PROFILE_SYNC \}\);/);
