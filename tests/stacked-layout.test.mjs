...
test('layout is pure, frozen, pixel-positioned, centred and exposes both pinned names', () => {
  ...
  for (const key of ['slot', 'x', 'y']) assert(Number.isInteger(slots[0][key]), key);
  assert(slots[0].scale > 0);
  const [rect] = computeBoardRects(slots);
  assert(Math.abs(rect.x + rect.width / 2 - input.viewportWidth / 2) <= .5);
  assert(Math.abs(rect.y + rect.height / 2 - input.viewportHeight / 2) <= .5);
});

test('two wide boards use the measured formulas, ascending slots and never overlap', () => {
  for (const [viewportWidth, viewportHeight] of [[1920,1080],[1280,720],[900,500],[1600,600],[900,600]]) {
    const expectedScale = Math.min(viewportHeight * .9 / 640, (viewportWidth * .92 - Math.round(viewportWidth * .04)) / 1024);
...
test('safe areas constrain and centre the authored box inside usable pixels', () => {
  ...
  assert(Math.abs(rect.x+rect.width/2-(60+(1180/2))) <= .5);
});
