...
test('every worldTour a scene requests has a spawn in the runtime', () => {
  const views = createWorldTourSpawns(buildAuthoredPointOfInterestPlacements(LEVEL_ONE_WORLD.pointsOfInterest));
  ...
  for (const scene of VISUAL_SCENES) {
    ...
    if (collectibleTours.has(tour)) {
      assert.ok(views[tour], 'runtime collectible-tour generator is missing');
      continue;
    }
    assert.ok(
      Boolean(views[tour]),
