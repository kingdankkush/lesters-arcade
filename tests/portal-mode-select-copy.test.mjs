...
test('the preview cards promise no publishing and the launch cards state the fee and the check', () => {
  for (const gameId of SITE_COPY_GAMES) {
    ...
    assert.doesNotMatch(text, /Publish your score|zkLTC|LiteForge|0\.1001|replayed/i, `${gameId} preview`);
  }
  const hmh = renderModeSelect({ gameId: 'lester-blaster', portalCopy: launch });
  assert.match(hmh.ranked, /^0\.1001 testnet zkLTC per run\. The arcade server plausibility-checks your run \(it is not replayed\) and publishes it on LitVM\.$/);
  const chikun = renderModeSelect({ gameId: 'chikun', portalCopy: launch });
  assert.match(chikun.ranked, /^0\.1001 testnet zkLTC per run\. The arcade server replays your run from its inputs and publishes it on LitVM\.$/);
...
test('the prerendered mode-select blocks match what the SPA shows', () => {
  ...
  try {
    ...
    assert.match(readFileSync(join(dir, `discover/${slugOf('stacked')}.html`), 'utf8'), /<!-- copy:mode-ranked:start -->0\.1001 testnet zkLTC per run\. The arcade server checks your run and publishes it on LitVM\.<!-- copy:mode-ranked:end -->/);
  } finally {
