...
test('WO-55 Chikun vertical slice has deterministic flap/fork/coin scoring rules', () => {
  ...
  assert.equal(config.rules.difficulty.minimumGapHeight, 238);
  assert.equal(config.rules.difficulty.maximumScrollPixelsPerTick, 3.35);
...
test('ranked Chikun cabinet binds simulation to the parent-issued seed and session metadata', () => {
  ...
  assert.equal(ctx.buildHash, 'site-1.4.0:game-1.4.0:cabinet-0.5.0');
