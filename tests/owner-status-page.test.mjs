...
function healthyReport(overrides = {}) {
  const base = {
    ...
    queue: { pending: 0, signed: 0, submitted: 1, failed: 0, dead: 0, oldestUnconfirmedAgeSeconds: 40 },
...
test('each threshold warns just past its limit', () => {
  ...
  assert.match(healthWarnings(healthyReport({ relayer: { estimatedSettlesLeft: 49 } }))[0].text, new RegExp(`about 49 more settles .*Send testnet zkLTC to ${RELAYER}`));
  ...
  assert.deepEqual(ids(healthyReport({ queue: { dead: 1 } })), ['queue-dead']);
  assert.match(healthWarnings(healthyReport({ queue: { dead: 1 } }))[0].text, /^1 run is a dead letter .*node scripts\/requeue-dead-letters\.mjs/);
  assert.match(healthWarnings(healthyReport({ queue: { dead: 3 } }))[0].text, /^3 runs are dead letters/);
  ...
  const degraded = healthyReport({
    degraded: true, degradedParts: ['queue', 'relayer-balance'],
    relayer: { balanceWei: null, estimatedSettlesLeft: null }, queue: { pending: null, signed: null, submitted: null, failed: null, dead: null, oldestUnconfirmedAgeSeconds: null },
  });
...
test('the controller keeps the last report when a refresh fails and never overlaps loads', async () => {
  const answers = [healthyReport({ queue: { dead: 2 } }), null];
  let calls = 0;
...
test('the mounted page renders the numbers and warnings through the DOM', async () => {
  ...
  const intervals = [];
  const report = healthyReport({ relayer: { estimatedSettlesLeft: 12, balanceWei: '8550000000000000' }, queue: { failed: 1, dead: 1, oldestUnconfirmedAgeSeconds: 900 }, crons: { indexChain: { lastErrorAt: '2026-09-24T17:59:00.000Z', lastErrorCode: 'chain-read-failed', failures: 1 } } });
  const responses = [report, healthyReport()];
  const fetchImpl = async () => ({ ok: true, status: 200, json: async () => responses.shift() });
  const { controller, ready } = mountStatusPage(page.doc, { setInterval: (fn, ms) => intervals.push([fn, ms]) }, { fetchImpl });
  ...
  assert.deepEqual(page.rows('status-queue'), [
    ...
    ['Failed (retrying)', '1', 'warn'], ['Dead letters', '1', 'bad'], ['Oldest unpublished', '15 min', 'bad'],
  ]);
  ...
  assert.match(page.byId.get('status-loaded').textContent, /^Checked 2026-09-24 18:00 UTC \(0 s ago\)\. The API is cached for up to 30 s\.$/);
