...
test('migration 1 creates every table, index and constraint', async () => {
  ...
  try {
    ...
    assert.deepEqual(result, { version: 3, applied: [1, 2, 3] }, 'a fresh database gets migration 1 and the additive migrations 2 and 3');
...
test('migrate is idempotent and records the version', async () => {
  ...
  try {
    ...
    assert.deepEqual(rows, [{ version: 1, name: '0001_ranked_index' }, { version: 2, name: '0002_cron_runs' }, { version: 3, name: '0003_weekly_jackpot' }]);
...
test('ensureSchema migrates only when behind, memoizes per schemaKey and forgets failures', async () => {
  ...
  try {
    assert.equal(await ensureSchema(spy), 3);
    ...
    fail = false;
    assert.equal(await ensureSchema(flaky), 3, 'a failure clears the memo');
  } finally {
    await db.close();
  }
});
...
test('migrate retries a version once when a concurrent creator wins the catalog race', async () => {
  ...
  try {
    assert.deepEqual(await migrate(racing), { version: 3, applied: [1, 2, 3] });
