...
test('WO-58 profile v2 model builds trophy room, session feed, achievements, collection, privacy, and editing controls', async () => {
  ...
  const resultB = recordScore(state, sessionB, chikunRun.score, {
    ...
    nearMisses: chikunRun.nearMisses,
    bestCombo: chikunRun.bestCombo,
...
test('ranked sessions use one UUID-backed canonical id across routing and evidence', () => {
  ...
  assert.equal(session.canonicalContext.buildHash, `site-1.8.5:game-1.8.5:cabinet-${HMH_CABINET_VERSION}`);
...
test('parent session allocator issues deterministic seed, build, and season bindings for every cabinet', () => {
  ...
  assert.equal(a.buildHash, 'site-1.8.5:game-1.8.5:cabinet-0.9.0');
...
test('Chikun mode selection uses canonical title, supplied production art, and truthful score rails', () => {
  ...
  // The Free card shows to signed-in players too (polish-2 review).
  for (const descriptor of [hardMoneyHeroes, chikun]) assert.doesNotMatch(descriptor.free.copy, /guest/i);
