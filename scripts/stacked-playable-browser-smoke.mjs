...
try {
  ...
  for (const { name, mobile, width, height } of servedLive === true ? [] : [
    ...
    for (const expected of [/Ranked · Entry/i, `Entry ${RANKED_ENTRY_FEE_ZKLTC} zkLTC`, /Publishing/, `${RANKED_SETTLEMENT_GAS_RESERVE_ZKLTC} zkLTC`, `Total ${RANKED_ENTRY_TOTAL_ZKLTC} zkLTC`, /no transaction is sent/i]) {
