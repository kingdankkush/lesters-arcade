...
try {
  ...
  assert.deepEqual({ count: before.count, remaining: before.remaining }, { count: 0, remaining: 20 });
  ...
  assert.deepEqual({ count: collected.count, remaining: collected.remaining }, { count: 1, remaining: 19 });
  ...
  await page.waitForFunction(() => {
    ...
    return element?.dataset.weaponId === 'coin-blaster'
      ...
      && Number(element.dataset.collectibleRemaining) === 20;
  }, null, { timeout: 5000 });
  ...
  assert.deepEqual(reset, { count: 0, remaining: 20, active: '', weaponId: 'coin-blaster', handGrenades: 3 });
  ...
  assert.deepEqual({ count: timed.count, remaining: timed.remaining }, { count: 1, remaining: 19 });
  ...
  assert.deepEqual(timedReset, { count: 0, remaining: 20, active: '', weaponId: 'coin-blaster', handGrenades: 3 });
