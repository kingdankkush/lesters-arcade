...
try {
  ...
  assert.deepEqual({ count: collected.count, remaining: collected.remaining }, { count: 1, remaining: 8 });
  ...
  await page.waitForFunction(() => {
    ...
    return element?.dataset.weaponId === 'coin-blaster'
      ...
      && Number(element.dataset.collectibleRemaining) === 9;
  }, null, { timeout: 5000 });
  ...
  assert.deepEqual(reset, { count: 0, remaining: 9, active: '', weaponId: 'coin-blaster', handGrenades: 3 });
  ...
  assert.deepEqual({ count: timed.count, remaining: timed.remaining }, { count: 1, remaining: 8 });
