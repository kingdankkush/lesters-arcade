...
try {
  ...
  assert.deepEqual({ count: before.count, remaining: before.remaining }, { count: 0, remaining: 21 });
  ...
  assert.deepEqual({ count: collected.count, remaining: collected.remaining }, { count: 1, remaining: 20 });
  ...
  await page.waitForFunction(() => {
    ...
    return element?.dataset.weaponId === 'coin-blaster'
      ...
      && Number(element.dataset.collectibleRemaining) === 21;
  }, null, { timeout: 5000 });
  ...
  assert.deepEqual(reset, { count: 0, remaining: 21, active: '', weaponId: 'coin-blaster', handGrenades: 3 });
  ...
  assert.deepEqual({ count: timed.count, remaining: timed.remaining }, { count: 1, remaining: 20 });
  ...
  assert.deepEqual(timedReset, { count: 0, remaining: 21, active: '', weaponId: 'coin-blaster', handGrenades: 3 });
