...
import {CHIKUN_EVIDENCE_VERSION,flapTicksOf} from '../apps/portal/src/chikun-cabinet.mjs';
...
try{
 for(const [name,width,height] of [['desktop',1440,1000],['portrait',390,844],['landscape',844,390]]){
  ...
  const result=await frame.evaluate(()=>__messages.find(m=>m.type==='game:result')?.payload);assert.ok(result,'canonical result received');assert.equal(result.evidence.version,CHIKUN_EVIDENCE_VERSION);assert.equal(flapTicksOf(result.evidence).length,2);
  ...
  assert.deepEqual(errors,[]);reports.push({name,result:result.finalState,inputs:flapTicksOf(result.evidence),errors});await context.close();
 }
