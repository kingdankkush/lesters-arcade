...
try{
 for(const [name,width,height] of [['desktop',1440,1000],['portrait',390,844],['landscape',844,390]]){
  ...
  const result=await frame.evaluate(()=>__messages.find(m=>m.type==='game:result')?.payload);assert.ok(result,'canonical result received');assert.equal(result.evidence.version,'chikun-flap-evidence-v4');assert.equal(result.evidence.flapSteps.length,2);
